...
return(""+e[2]).replace(/^((?:https?|file|ftp|(chrome|moz|safari-web)-extension):\/\/.+)\/[^/]+$/,"$1")+"/"}return"/"}(),a[e]=t),t}}),o("4G3qk").register(JSON.parse('{"ceDhw":"index.0ee8619d.js","4M8Gf":"icon-add-order.d5dcde0a.png","kSZwU":"icon-cart.dd455062.png","8HtV0":"icon-order.5ea58f4f.png","kVJvh":"admin.04a53a14.js"}'));var d={};d=o("260Jh").getBundleURL("ceDhw")+o("4G3qk").resolve("4M8Gf");var i={};i=o("260Jh").getBundleURL("ceDhw")+o("4G3qk").resolve("kSZwU");class l{constructor(e){this.controller=e}// FOR TEST AND TBS - REMOVE FOR PROD
...
r.forEach(e=>{let r=document.createElement("div");r.classList.add("popup-item-wrapper");let n=document.createElement("label");n.setAttribute("for",e.name),n.innerText=e.label;let a=document.createElement("input");a.setAttribute("type",e.type),a.setAttribute("name",e.name),a.setAttribute("placeholder",e.placeholder),// Append the label and input to the item wrapper
...
let r=document.createElement("button");return r.classList.add("update-btn"),r.setAttribute("id","updatePaymentMethodBtn"),r.innerText="Update",t.appendChild(r),t}}var u=o("9zRuU");class h{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelProducts is connected!")}}class C{constructor(e){this.controller=e}checkModuleLinkage=()=>{console.log("HELLO, ModelCart is connected!")}}class g{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelOrders is connected!")}}class L{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelUser is connected!")};getSessionIdCustomerByCustomerId=(e,t)=>{let r=t.filter(t=>t.custId===e);return r[0]}}var E=o("ljOiA");// Init database instance (dataBase):
let y={},I="",P={},f={},b=[],_=[],T={},w={},v=document.createElement("div");v.innerHTML="";let B=document.createElement("div");B.innerHTML="";let O=document.createElement("div");O.innerHTML="";// Init cross-ref Map: image - url
let k=new Map,D=new class{constructor(){this.viewProducts=new l(this),this.viewCart=new s(this),this.viewOrders=new p(this),this.viewPopup=new m,this.modelFirebase=new u.ModelFirebase,this.modelProducts=new h,this.modelCart=new C(this),this.modelOrders=new g,this.modelUser=new L,this.bodyNode=document.querySelector("body"),this.containerLeftNode=document.getElementById("containerLeft"),this.containerRightNode=document.getElementById("containerRight"),this.containerPupupNode=document.getElementById("containerPupup"),this.popupContentNode=document.getElementById("popupContent")}initializeAppMain=async()=>{for(let e of(// TEST AND TBS - REMOVE IN PRODUCTION
...
k.set(e.itemImg,t)}// RUN DISCLAIMER FIRST
...
I=y.usersData[0].userId),console.log("[*** sessionIdUser ***] initiated: ",// Assign to sessionIdUser a first object/document from DB (usersData[0])
...
_=this.getCartItemsByUserID(I),console.log(`[*** sessionIdCartItems ***] for User ID ${I}: `,_),// 2. Set "sessionIdOrders" by sesstion's userId (sessionIdNumber):
b=this.getOrdersByUserID(I),console.log(`[*** sessionIdOrders ***] for User ID ${I}: `,b),// 3. Set "sessionIdCartDetails":
w=this.getSessionIdCartDetails(),console.log(`[*** sessionIdCartDetails ***] for User ID ${I}: `,w),// Left Container - Render product items
this.renderProductItemsList(y.productItems),// Right Container - Render Cart Summary
this.renderCartAndOrdersSummary(_,b),this.attachEventListenrs()};// TO-DO: check where it is used?
...
getSessionIdCartDetails=()=>{let e={userId:P.userId,userName:P.userName,userEmail:P.userEmail,userPhone:P.userPhone,custId:f.custId,custLegalName:f.custLegalName,custBillToAddress:f.custBillToAddress,custHandlingFee:f.custHandlingFee,paymentMethod:f.paymentMethod};return e};// Reset Session Cart Items (when order placed)
resetSessionIdCartItems=()=>{_=[]};// Revert back from map an image URL by image name
getUrlByNameLocal=e=>k.get(e);// Set "sessionIdCartItems" by sesstion's userId (sessionIdNumber):
getCartItemsByUserID=e=>y.cartsData.filter(t=>t.userId===e);// Set "sessionIdOrders" by sesstion's userId and sorted by orderDate:
getOrdersByUserID=e=>y.ordersData.filter(t=>t.userId===e).sort((e,t)=>e.orderDate-t.orderDate);// Get "orderDetailsToDisplay" by orderNumber:
getOrderDetailsToDisplay=e=>y.ordersData.filter(t=>t.orderId===e)[0];// Get "orderItemsToDisplay" by orderNumber:
getOrderItemsToDisplay=e=>y.orderItemsData.filter(t=>t.orderId===e);getImageByProductId=e=>{let t=y.productItems.filter(t=>t.itemId===e);return t[0].itemImg};getPriceByProductId=(e,t)=>{let r=e.filter(e=>e.itemId===t);return r[0].itemPrice};getProdIdFromElementId=e=>e.split("_")[1];// Get product/object from "dataBase.productItems" product ID (itemId):
...
console.log("Received command to call for Price Variable update"),this.updateCartPriceVariable(_,T,y.productItems,f.custHandlingFee)};// MIGHT BE A PART OF MODEL:
...
e.appendChild(this.viewOrders.createPaymentMethodContentBlock(w,!0,E.paymentMethods));let t=this.viewOrders.createCheckOutCartWrapper(),r=document.createElement("div");for(let e of(r.classList="checkout-cart-items",_)){let t=this.getProductObjectById(e.itemId);// Third argument ("checkout-cart-item") is a classList to pass to 'div' element
...
r.appendChild(this.viewOrders.createPaymentMethodContentBlock(e,!1,E.paymentMethods));let n=this.viewOrders.createCheckOutCartWrapper(),a=document.createElement("div");for(let e of(a.classList="checkout-cart-items",t)){let t=this.getProductObjectById(e.itemId);// Third argument ("checkout-cart-item") is a classList to pass to 'div' element
// Fourth argument ("true") is for not to create a "qty update" feature
...
this.renderCartAndOrdersSummary(_,b)};// "Add to Cart" button at detailed product card (page)
...
let t=this.getProdIdFromElementId(e.id),r=this.getDocIdFromArrayByItemId(_,t);r?this.handleQtyChangeBtn(e,1):this.addProduct(t,I),this.renderCartAndOrdersSummary(_,b)};// "Go to Cart" button at right panel or checkout panel
handleGotoCartBtn=()=>{// TEST-TBS REMOVE IN PROD
console.log("Received command to display CART."),this.updateCartPriceVariable(_,T,y.productItems,f.custHandlingFee),this.renderCartSummary(_),this.renderCartSummaryWithPrice(_,T)};// Handle Order Placement
handleOrderPlacement=async()=>{if(// TEST-TBS REMOVE IN PROD
console.log("Received command to PLACE ORDER (HANDLER CONNECTED)."),0===T.priceTotal){this.activatePopupMessage(E.errorEmptyCart);return}// 1.A. Prepare newOrderDetails to pass to Firestore
...
let t=JSON.parse(JSON.stringify(_)),r=JSON.parse(JSON.stringify(e.orderId));for(let e in // TEST-TBS REMOVE IN PROD
...
for(let e in _){console.log("[*** sessionIdCartItems[cartItem].docId ***] to delete from cartsData: ",_[e].docId);try{this.handleDeleteDocFromFirestore("cartsData",_[e].docId)}catch(e){console.error("Error deleting sessionIdCartItems from Firestore: ",e)}}// 3.B. Reset sessionIdCartItems
...
console.log("Function: 'handleQtyChangeBtn()': update QTY of prod ID: ",e.id),console.log("'sessionIdCartItems': ",_),console.log("itemId: ",r);// product/object to update:
let n=JSON.parse(JSON.stringify(_.find(e=>e.itemId===r)));if(!n){console.log("Product/object not found");return}// remove product from array:
let a=_.filter(e=>e.itemId!==r);n.qty+t>0?(this.updateProduct(a,n,t),this.updateCartDOM(r,n)):(this.deleteProduct(a,n),this.handleGotoCartBtn())};// Update payment method if new one is selected by User
updatePaymentMethodInSession=()=>{let e=this.handleRadioButtonSelection();null!==e?w.paymentMethod!==e?(// Compare with current value
w.paymentMethod=e,console.log(`Updated paymentMethod to ${e}`)):console.log("paymentMethod is the same. No update needed."):console.log("No radio button selected. Cannot update paymentMethod.")};addProduct=(e,t)=>{let r={docId:this.handleNewIdGeneration(),itemId:e,qty:1,userId:t};console.log("MODEL: try to add to database.cartsData a [*** newProductToCart *** ]: ",r),this.handleAddDocToFirestore("cartsData",r),_.push(r),console.log("Updated [*** sessionIdCartItems ***]: ",_)};updateProduct=(e,t,r)=>{// Modify product/object
...
_=e};deleteProduct=(e,t)=>{// Delete product from Firestore
this.handleDeleteDocFromFirestore("cartsData",t.docId),// Update global variable
_=e};updateCartDOM=(e,t)=>{//  update/re-render 'qty' in user's cart (DOM)
...
console.log("Got comand to ACTIVATE WELCOME DISCLAIMER"),this.popupContentNode.innerHTML="";let e=this.viewPopup.createPopupMessage(E.welcomeDisclaimer);this.popupContentNode.appendChild(e),this.togglePopup()};activatePopupMessage=e=>{// Clear Popup Content
...
console.log("Got comand to ACTIVATE POPUP"),this.popupContentNode.innerHTML="";let e=this.viewPopup.createPopupUpdateAddress(w);this.popupContentNode.appendChild(e),this.togglePopup()};activatePopupUpdatePaymentMethod=()=>{// Clear Popup Content
console.log("Got comand to ACTIVATE POPUP"),this.popupContentNode.innerHTML="",this.popupContentNode.appendChild(this.viewPopup.createPopupUpdatePaymentMethod(E.paymentMethods)),this.togglePopup()};togglePopup=()=>{this.bodyNode.classList.toggle("body-fixed"),this.containerPupupNode.classList.toggle("popup-container-open")};// Function to handle radio button selection in POPUP SECTION
