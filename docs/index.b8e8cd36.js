...
o.appendChild(i),l.appendChild(s),l.appendChild(p),l.appendChild(m),l.appendChild(u),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(y),l.appendChild(E),l.appendChild(I),P.appendChild(_),P.appendChild(b),c.appendChild(l),c.appendChild(P),a.appendChild(o),a.appendChild(c),f.appendChild(v),f.appendChild(T),r.appendChild(n),r.appendChild(a),r.appendChild(f),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
...
createUpdateButtonBlock=()=>{let e=document.createElement("div");e.classList="update-btn-wrapper";let t=document.createElement("button");return t.classList="update-btn",t.innerText="Update",e.appendChild(t),e};createPopupUpdateAddress=()=>{let e=`                
                ...
                </div>`;return e};createPopupUpdatePaymentMethod=()=>{let e=`
        ...
        `;return e}}class m{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ViewAdmin is connected!")}}class u{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ViewSuperadmin is connected!")}}var h=o("keq33");class C{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelProducts is connected!")}}class g{constructor(e){this.controller=e}checkModuleLinkage=()=>{console.log("HELLO, ModelCart is connected!")}}class L{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelOrders is connected!")}}class y{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelUser is connected!")};getSessionIdCustomerByCustomerId=(e,t)=>{let r=t.filter(t=>t.custId===e);return r[0]}}var E=o("1ggDV");// Init database instance (dataBase):
let I={},P="",_={},b={},f=[],v=[],T={},w={};// let newOrderDetails = {
//     // userId: "1001",
...
// };
// Init HTML variables:
const O=document.createElement("div");O.innerHTML="";const B=document.createElement("div");B.innerHTML="";const k=document.createElement("div");k.innerHTML="";// Init cross-ref Map: image - url
const D=new Map,x=new class{constructor(){this.viewProducts=new c(this),this.viewCart=new s(this),this.viewOrders=new p(this),this.viewAdmin=new m,this.viewSuperadmin=new u,this.modelFirebase=new h.ModelFirebase,this.modelProducts=new C,this.modelCart=new g(this),this.modelOrders=new L,this.modelUser=new y,this.bodyNode=document.querySelector("body"),this.containerLeftNode=document.getElementById("containerLeft"),this.containerRightNode=document.getElementById("containerRight"),this.containerPupupNode=document.getElementById("containerPupup"),this.popupContentNode=document.getElementById("popupContent")}initializeAppMain=async()=>{for(let e of(// TEST AND TBS - REMOVE IN PRODUCTION
...
_=I.usersData[0]),console.log("[*** sessionIdCustomer ***] initiated: ",// Assign to sessionIdCustomer an object/document associated with sesstionIdUser (by userId)
...
v=this.getCartItemsByUserID(P),console.log(`[*** sessionIdCartItems ***] for User ID ${P}: `,v),// 2. Set "sessionIdOrders" by sesstion's userId (sessionIdNumber):
f=this.getOrdersByUserID(P),console.log(`[*** sessionIdOrders ***] for User ID ${P}: `,f),// 3. Set "sessionIdCartDetails":
w=this.getSessionIdCartDetails(),console.log(`[*** sessionIdCartDetails ***] for User ID ${P}: `,w),// ALERT - POPOUP DISCLAMER
...
this.renderCartAndOrdersSummary(v,f),this.attachEventListenrs();// TEST-TBS ONLY:
...
getUpdateLocalDataBase=async()=>{for(let e of(console.log("Get or Update data from Firebase and keep in local dataBase{}."),E.dbCollectionNames))I[e]=await this.modelFirebase.get(e),console.log(`[*** dataBase.${e} ***] :`,I[e])};deleteObjectFromArrayByDocId=(e,t)=>{let r=e.filter(e=>e.docId===t);return -1!==r?(e.splice(r,1),console.log("Deleted element/document with docId = ",t)):console.log("Not found element/document with docId = ",t),e};// Assign to sessionIdCustomer an object/document associated with sesstionIdUser (by userId)
getSessionIdCustomerData=()=>b=this.modelUser.getSessionIdCustomerByCustomerId(_.custId,I.customersData);// Set "sessionIdCartDetails":
getSessionIdCartDetails=()=>{let e={userId:_.userId,userName:_.userName,userEmail:_.userEmail,userPhone:_.userPhone,custId:b.custId,custLegalName:b.custLegalName,custBillToAddress:b.custBillToAddress,custHandlingFee:b.custHandlingFee,paymentMethod:b.paymentMethod};return e};// Reset Session Cart Items (when order placed)
resetSessionIdCartItems=()=>{v=[]};// Revert back from map an image URL by image name
...
getOrdersByUserID=e=>I.ordersData.filter(t=>t.userId===e);// Get "orderDetailsToDisplay" by orderNumber:
getOrderDetailsToDisplay=e=>I.ordersData.filter(t=>t.orderId===e)[0];// Get "orderItemsToDisplay" by orderNumber:
getOrderItemsToDisplay=e=>I.orderItemsData.filter(t=>t.orderId===e);getImageByProductId=e=>{let t=I.productItems.filter(t=>t.itemId===e);// TEST-TBS - REMOVE IN PROD
// console.log("productObject: ", productObject);
...
this.updateCartPriceVariable(v,T,I.productItems,b.custHandlingFee)};// MIGHT BE A PART OF MODEL:
updateCartPriceVariable=(e,t,r,n)=>{let a=0,o=0;for(let n of(// TO-DO: CHECK, if it is required here. Delete for now:
console.log("MODEL - [*** sessionIdCartItems ***]: ",e),console.log("MODEL - [*** sessionIdCartPrice ***]: ",t),console.log("MODEL - [*** dataBase.productItems ***]: ",I.productItems),console.log("MODEL - [*** sessionIdCustome ***]: ",b),e))a+=n.qty*this.getPriceByProductId(r,n.itemId);// TO-DO: CHECK, if it is required here. Delete for now:
// this.controller.handleViewOfTotalPrceInCart(sessionIdCartPrice.priceTotal);
return o=a*n/100,t.priceItems=a,t.priceHandling=o,t.priceTotal=a+o,console.log("new sessionIdCartPrice: ",t),console.log("for sessionIdCartItems: ",e),t};// Left Container - Render product items
...
e.appendChild(this.viewOrders.createBillToAddressContentBlock(w,!0)),// Second argument ("true") is to create "Update" button
e.appendChild(this.viewOrders.createPaymentMethodContentBlock(w,!0));let t=this.viewOrders.createCheckOutCartWrapper(),r=document.createElement("div");for(let e of(r.classList="checkout-cart-items",v)){let t=this.getProductObjectById(e.itemId);// Third argument ("checkout-cart-item") is a classList to pass to 'div' element
...
renderCheckOutPricePanel=()=>{this.clearContainerRight(),this.containerRightNode.appendChild(this.viewOrders.createOrderSummaryPriceBlock(T,!0))};// Right Container - Render Order Price Panel
...
this.renderCartAndOrdersSummary(v,f)};// "Add to Cart" button at detailed product card (page)
...
let t=this.getProdIdFromElementId(e.id),r=this.getDocIdFromArrayByItemId(v,t);r?this.handleQtyChangeBtn(e,1):this.addProduct(t,P),this.renderCartAndOrdersSummary(v,f)};// "Go to Cart" button at right panel or checkout panel
handleGotoCartBtn=()=>{// TEST-TBS REMOVE IN PROD
console.log("Received command to display CART."),this.updateCartPriceVariable(v,T,I.productItems,b.custHandlingFee),this.renderCartSummary(v),this.renderCartSummaryWithPrice(v,T)};// Handle Order Placement
...
let e=JSON.parse(JSON.stringify(this.modelFirebase.setNewOrderDetails(w,T)));// TEST-TBS REMOVE IN PROD
...
let t=JSON.parse(JSON.stringify(v)),r=JSON.parse(JSON.stringify(e.orderId));for(let e in // TEST-TBS REMOVE IN PROD
...
for(let e in v){console.log("[*** sessionIdCartItems[cartItem].docId ***] to delete from cartsData: ",v[e].docId);try{this.handleDeleteDocFromFirestore("cartsData",v[e].docId)}catch(e){console.error("Error deleting sessionIdCartItems from Firestore: ",e)}}// 3.B. Reset sessionIdCartItems
...
console.log("Function: 'handleQtyChangeBtn()': update QTY of prod ID: ",e.id),console.log("'sessionIdCartItems': ",v),console.log("itemId: ",r);// product/object to update:
let n=JSON.parse(JSON.stringify(v.find(e=>e.itemId===r)));if(!n){console.log("Product/object not found");return}// remove product from array:
let a=v.filter(e=>e.itemId!==r);n.qty+t>0?(this.updateProduct(a,n,t),this.updateCartDOM(r,n)):(this.deleteProduct(a,n),this.handleGotoCartBtn())};addProduct=(e,t)=>{let r={docId:this.handleNewIdGeneration(),itemId:e,qty:1,userId:t};console.log("MODEL: try to add to database.cartsData a [*** newProductToCart *** ]: ",r),this.handleAddDocToFirestore("cartsData",r),v.push(r),console.log("Updated [*** sessionIdCartItems ***]: ",v)};updateProduct=(e,t,r)=>{// Modify product/object
...
v=e};deleteProduct=(e,t)=>{// Delete product from Firestore
this.handleDeleteDocFromFirestore("cartsData",t.docId),// Update global variable
v=e};updateCartDOM=(e,t)=>{//  update/re-render 'qty' in user's cart (DOM)
...
this.handleViewOfTotalPrceInCart(T.priceTotal)};// Update product QTY in CART in 2 locations: price and selector
...
activatePopupUpdateAddress=()=>{// Clear Popup Content
console.log("Got comand to ACTIVATE POPUP"),this.popupContentNode.innerHTML="",this.popupContentNode.innerHTML=this.viewOrders.createPopupUpdateAddress(),this.togglePopup()};activatePopupUpdatePaymentMethod=()=>{// Clear Popup Content
console.log("Got comand to ACTIVATE POPUP"),this.popupContentNode.innerHTML="",this.popupContentNode.innerHTML=this.viewOrders.createPopupUpdatePaymentMethod(),this.togglePopup()};togglePopup=()=>{this.bodyNode.classList.toggle("body-fixed"),this.containerPupupNode.classList.toggle("popup-container-open")};// *** BUTTON CLICK HANDLERS ***
