...
return(""+e[2]).replace(/^((?:https?|file|ftp|(chrome|moz|safari-web)-extension):\/\/.+)\/[^/]+$/,"$1")+"/"}return"/"}(),a[e]=t),t}}),o("4G3qk").register(JSON.parse('{"ceDhw":"index.901c3210.js","4M8Gf":"icon-add-order.d5dcde0a.png","kSZwU":"icon-cart.dd455062.png","8HtV0":"icon-order.5ea58f4f.png","kVJvh":"admin.e4952735.js"}'));var d={};d=o("260Jh").getBundleURL("ceDhw")+o("4G3qk").resolve("4M8Gf");var i={};i=o("260Jh").getBundleURL("ceDhw")+o("4G3qk").resolve("kSZwU");class c{constructor(e){this.controller=e}// FOR TEST AND TBS - REMOVE FOR PROD
...
o.appendChild(i),l.appendChild(s),l.appendChild(p),l.appendChild(m),l.appendChild(u),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(E),l.appendChild(y),l.appendChild(I),P.appendChild(f),P.appendChild(_),c.appendChild(l),c.appendChild(P),a.appendChild(o),a.appendChild(c),b.appendChild(T),b.appendChild(w),r.appendChild(n),r.appendChild(a),r.appendChild(b),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
...
let t=document.createElement("div");t.classList.add("popup-payment-method"),// Define the paymentMethods object
...
b=this.getOrdersByUserID(P),console.log(`[*** sessionIdOrders ***] for User ID ${P}: `,b),// 3. Set "sessionIdCartDetails":
O=this.getSessionIdCartDetails(),console.log(`[*** sessionIdCartDetails ***] for User ID ${P}: `,O),// ALERT - POPOUP DISCLAMER
...
this.renderCartAndOrdersSummary(T,b),this.attachEventListenrs();// TEST-TBS ONLY:
...
getSessionIdCustomerData=()=>_=this.modelUser.getSessionIdCustomerByCustomerId(f.custId,I.customersData);// Set "sessionIdCartDetails":
getSessionIdCartDetails=()=>{let e={userId:f.userId,userName:f.userName,userEmail:f.userEmail,userPhone:f.userPhone,custId:_.custId,custLegalName:_.custLegalName,custBillToAddress:_.custBillToAddress,custHandlingFee:_.custHandlingFee,paymentMethod:_.paymentMethod};return e};// Reset Session Cart Items (when order placed)
...
this.updateCartPriceVariable(T,w,I.productItems,_.custHandlingFee)};// MIGHT BE A PART OF MODEL:
updateCartPriceVariable=(e,t,r,n)=>{let a=0,o=0;for(let n of(// TO-DO: CHECK, if it is required here. Delete for now:
console.log("MODEL - [*** sessionIdCartItems ***]: ",e),console.log("MODEL - [*** sessionIdCartPrice ***]: ",t),console.log("MODEL - [*** dataBase.productItems ***]: ",I.productItems),console.log("MODEL - [*** sessionIdCustome ***]: ",_),e))a+=n.qty*this.getPriceByProductId(r,n.itemId);// TO-DO: CHECK, if it is required here. Delete for now:
...
e.appendChild(this.viewOrders.createBillToAddressContentBlock(O,!0)),// Second argument ("true") is to create "Update" button
e.appendChild(this.viewOrders.createPaymentMethodContentBlock(O,!0));let t=this.viewOrders.createCheckOutCartWrapper(),r=document.createElement("div");for(let e of(r.classList="checkout-cart-items",T)){let t=this.getProductObjectById(e.itemId);// Third argument ("checkout-cart-item") is a classList to pass to 'div' element
...
this.renderCartAndOrdersSummary(T,b)};// "Add to Cart" button at detailed product card (page)
...
let t=this.getProdIdFromElementId(e.id),r=this.getDocIdFromArrayByItemId(T,t);r?this.handleQtyChangeBtn(e,1):this.addProduct(t,P),this.renderCartAndOrdersSummary(T,b)};// "Go to Cart" button at right panel or checkout panel
handleGotoCartBtn=()=>{// TEST-TBS REMOVE IN PROD
console.log("Received command to display CART."),this.updateCartPriceVariable(T,w,I.productItems,_.custHandlingFee),this.renderCartSummary(T),this.renderCartSummaryWithPrice(T,w)};// Handle Order Placement
...
let e=JSON.parse(JSON.stringify(this.modelFirebase.setNewOrderDetails(O,w)));// TEST-TBS REMOVE IN PROD
