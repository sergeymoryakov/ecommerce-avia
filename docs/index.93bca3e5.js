...
class h{constructor(){this.containerLeftNode=document.getElementById("containerLeft"),this.containerRightNode=document.getElementById("containerRight"),// CHECK - WHERE IT HAS TO BE:
...
for(let r of(i.appendChild(n),e)){let e=this.createDataRow(r,t);i.appendChild(e)}return i};renderTable=(e,t)=>{let i=document.createElement("div");i.classList="db-section";let n=document.createElement("h2");n.classList="db-section__title",n.id=`${t}`,n.innerText=t;let r=this.createTable(e,t);r.classList="db-section__table",i.appendChild(n),i.appendChild(r),// FOR TBS-TEST RENDER TABLE ELEMENTS:
// console.log(tableWrapper);
this.containerLeftNode.appendChild(i)};createMenuBtn=e=>{let t=document.createElement("li");t.classList="menu-btn-wrapper";let i=document.createElement("a");return i.classList="menu-btn",i.href=`#${e}`,i.innerText=`${e}`,t.appendChild(i),t};createMenu=(e,t,i,n,r)=>{// Init menu wrapper:
...
class u{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, AdminModel is connected!")};// New ID Generator:
...
return eF.call(this,e,0)}}}!function(){try{eF="function"==typeof setTimeout?setTimeout:f}catch(e){eF=f}try{eV="function"==typeof clearTimeout?clearTimeout:m}catch(e){eV=m}}();var g=[],y=!1,v=-1;function w(){y&&eU&&(y=!1,eU.length?g=eU.concat(g):v=-1,g.length&&b())}function b(){if(!y){var e=p(w);y=!0;for(var t=g.length;t;){for(eU=g,g=[];++v<t;)eU&&eU[v].run();v=-1,t=g.length}eU=null,y=!1,function(e){if(eV===clearTimeout)return clearTimeout(e);// if clearTimeout wasn't available but was latter defined
 ...
 */const A=function(e){let t=T(e);return I.encodeByteArray(t,!0)},D=function(e){// Use base64url encoding and remove padding in the end (dot characters).
 ...
 */class e_{constructor(e,t,i){this._isDeleted=!1,this._options=Object.assign({},e),this._config=Object.assign({},t),this._name=t.name,this._automaticDataCollectionEnabled=t.automaticDataCollectionEnabled,this._container=i,this.container.addComponent(new $("app",()=>this,"PUBLIC"/* ComponentType.PUBLIC */))}get automaticDataCollectionEnabled(){return this.checkDestroyed(),this._automaticDataCollectionEnabled}set automaticDataCollectionEnabled(e){this.checkDestroyed(),this._automaticDataCollectionEnabled=e}get name(){return this.checkDestroyed(),this._name}get options(){return this.checkDestroyed(),this._options}get config(){return this.checkDestroyed(),this._config}get container(){return this._container}get isDeleted(){return this._isDeleted}set isDeleted(e){this._isDeleted=e}/**
     ...
     */checkDestroyed(){if(this.isDeleted)throw eE.create("app-deleted"/* AppError.APP_DELETED */,{appName:this._name})}}function eT(e,t={}){let i=e;if("object"!=typeof t){let e=t;t={name:e}}let n=Object.assign({name:eg,automaticDataCollectionEnabled:!1},t),r=n.name;if("string"!=typeof r||!r)throw eE.create("bad-app-name"/* AppError.BAD_APP_NAME */,{appName:String(r)});if(i||(i=M()),!i)throw eE.create("no-options"/* AppError.NO_OPTIONS */);let s=ev.get(r);if(s){// return the existing app if options and config deep equal the ones in the existing app.
if(q(i,s.options)&&q(n,s.config))return s;throw eE.create("duplicate-app"/* AppError.DUPLICATE_APP */,{appName:r})}let a=new H(r);for(let e of ew.values())a.addComponent(e);let o=new e_(i,n,a);return ev.set(r,o),o}/**
...
let r=null!==(n=ey[e])&&void 0!==n?n:e;i&&(r+=`-${i}`);let s=r.match(/\s|\//),a=t.match(/\s|\//);if(s||a){let e=[`Unable to register library "${r}" with version "${t}":`];s&&e.push(`library name "${r}" contains illegal characters (whitespace or "/")`),s&&a&&e.push("and"),a&&e.push(`version name "${t}" contains illegal characters (whitespace or "/")`),ep.warn(e.join(" "));return}eb(new $(`${r}-version`,()=>({library:r,version:t}),"VERSION"/* ComponentType.VERSION */))}const eI="firebase-heartbeat-store";let eS=null;function eA(){return eS||(eS=/**
 * Open a database.
 *
 * @param name Name of the database.
 * @param version Schema version.
 * @param callbacks Additional callbacks.
 */(function(e,t,{blocked:i,upgrade:n,blocking:r,terminated:s}={}){let a=indexedDB.open(e,1),o=ea(a);return n&&a.addEventListener("upgradeneeded",e=>{n(ea(a.result),e.oldVersion,e.newVersion,ea(a.transaction),e)}),i&&a.addEventListener("blocked",e=>i(e.oldVersion,e.newVersion,e)),o.then(e=>{s&&e.addEventListener("close",()=>s()),r&&e.addEventListener("versionchange",e=>r(e.oldVersion,e.newVersion,e))}).catch(()=>{}),o})("firebase-heartbeat-database",0,{upgrade:(e,t)=>{0===t&&e.createObjectStore(eI)}}).catch(e=>{throw eE.create("idb-open"/* AppError.IDB_OPEN */,{originalErrorMessage:e.message})})),eS}async function eD(e){try{let t=await eA(),i=await t.transaction(eI).objectStore(eI).get(ek(e));return i}catch(e){if(e instanceof V)ep.warn(e.message);else{let t=eE.create("idb-get"/* AppError.IDB_GET */,{originalErrorMessage:null==e?void 0:e.message});ep.warn(t.message)}}}async function eN(e,t){try{let i=await eA(),n=i.transaction(eI,"readwrite"),r=n.objectStore(eI);await r.put(t,ek(e)),await n.done}catch(e){if(e instanceof V)ep.warn(e.message);else{let t=eE.create("idb-set"/* AppError.IDB_WRITE */,{originalErrorMessage:null==e?void 0:e.message});ep.warn(t.message)}}}function ek(e){return`${e.name}!${e.options.appId}`}class eL{constructor(e){this.container=e,/**
...
return D(JSON.stringify({version:2,heartbeats:e})).length}eb(new $("platform-logger",e=>new ed(e),"PRIVATE"/* ComponentType.PRIVATE */)),eb(new $("heartbeat",e=>new eL(e),"PRIVATE"/* ComponentType.PRIVATE */)),// Register `app` package.
 ...
 * @internal
 */class rE{getToken(){return Promise.resolve(null)}invalidateToken(){}start(e,t){// Fire with initial user.
...
let e=this.i,t=this.forceRefresh;return this.forceRefresh=!1,this.auth?this.auth.getToken(t).then(t=>// outstanding so the response is potentially for a previous user (which
    ...
    this.i!==e?(rd("FirebaseAuthCredentialsProvider","getToken aborted due to token change."),this.getToken()):t?("string"==typeof t.accessToken||rg(),new rb(t.accessToken,this.currentUser)):null):Promise.resolve(null)}invalidateToken(){this.forceRefresh=!0}shutdown(){this.auth&&this.auth.removeAuthTokenListener(this.o)}// Auth.getUid() can return null even with a user logged in. It is because
...
t.push(...i.split("/").filter(e=>e.length>0))}return new rP(t)}static emptyPath(){return new rP([])}}const rO=/^[_a-zA-Z][_a-zA-Z0-9]*$/;/**
 ...
 */function sC(e){for(let t of e.filters)if(t instanceof s_)return!1;return!0}class sI extends sE{constructor(e,t,i){super(e,t,i),this.key=rF.fromName(i.referenceValue)}matches(e){let t=rF.comparator(e.key,this.key);return this.matchesComparison(t)}}/** Filter that matches on key fields within an array. */class sS extends sE{constructor(e,t){super(e,"in"/* Operator.IN */,t),this.keys=sD("in"/* Operator.IN */,t)}matches(e){return this.keys.some(t=>t.isEqual(e.key))}}/** Filter that matches on key fields not present within an array. */class sA extends sE{constructor(e,t){super(e,"not-in"/* Operator.NOT_IN */,t),this.keys=sD("not-in"/* Operator.NOT_IN */,t)}matches(e){return!this.keys.some(t=>t.isEqual(e.key))}}function sD(e,t){var i;return((null===(i=t.arrayValue)||void 0===i?void 0:i.values)||[]).map(e=>rF.fromName(e.referenceValue))}/** A Filter that implements the array-contains operator. */class sN extends sE{constructor(e,t){super(e,"array-contains"/* Operator.ARRAY_CONTAINS */,t)}matches(e){let t=e.data.field(this.field);return sl(t)&&sn(t.arrayValue,this.value)}}/** A Filter that implements the IN operator. */class sk extends sE{constructor(e,t){super(e,"in"/* Operator.IN */,t)}matches(e){let t=e.data.field(this.field);return null!==t&&sn(this.value.arrayValue,t)}}/** A Filter that implements the not-in operator. */class sL extends sE{constructor(e,t){super(e,"not-in"/* Operator.NOT_IN */,t)}matches(e){if(sn(this.value.arrayValue,{nullValue:"NULL_VALUE"}))return!1;let t=e.data.field(this.field);return null!==t&&!sn(this.value.arrayValue,t)}}/** A Filter that implements the array-contains-any operator. */class sR extends sE{constructor(e,t){super(e,"array-contains-any"/* Operator.ARRAY_CONTAINS_ANY */,t)}matches(e){let t=e.data.field(this.field);return!(!sl(t)||!t.arrayValue.values)&&t.arrayValue.values.some(e=>sn(this.value.arrayValue,e))}}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * property.
 */function sP(e,t=null,i=[],n=[],r=null,s=null,a=null){return new sx(e,t,i,n,r,s,a)}function sO(e){if(null===e.he){let t=e.path.canonicalString();null!==e.collectionGroup&&(t+="|cg:"+e.collectionGroup),t+="|f:"+e.filters.map(e=>(function e(t){if(t instanceof sE)// the same description, such as the int 3 and the string "3". So we should
...
function aw(e){let t=new DataView(e.buffer),i=t.getUint32(0,/* littleEndian= */!0),n=t.getUint32(4,/* littleEndian= */!0),r=t.getUint32(8,/* littleEndian= */!0),s=t.getUint32(12,/* littleEndian= */!0);return[new ra([i,n],0),new ra([r,s],0)]}class ab{constructor(e,t,i){if(this.bitmap=e,this.padding=t,this.hashCount=i,t<0||t>=8)throw new aE(`Invalid padding: ${t}`);if(i<0||e.length>0&&0===this.hashCount)throw new aE(`Invalid hash count: ${i}`);if(0===e.length&&0!==t)throw new aE(`Invalid padding when bitmap length is 0: ${t}`);this.Ae=8*e.length-t,this.Re=ra.fromNumber(this.Ae)}// Calculate the ith hash value based on the hashed 64bit integers,
...
if(0===this.Ae)return!1;let t=av(e),[i,n]=aw(t);for(let e=0;e<this.hashCount;e++){let t=this.Ve(i,n,e);if(!this.me(t))return!1}return!0}/** Create bloom filter for testing purposes only. */static create(e,t,i){let n=new Uint8Array(Math.ceil(e/8)),r=new ab(n,e%8==0?0:8-e%8,t);return i.forEach(e=>r.insert(e)),r}insert(e){if(0===this.Ae)return;let t=av(e),[i,n]=aw(t);for(let e=0;e<this.hashCount;e++){let t=this.Ve(i,n,e);this.fe(t)}}fe(e){this.bitmap[Math.floor(e/8)]|=1<<e%8}}class aE extends Error{constructor(){super(...arguments),this.name="BloomFilterError"}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
i=new ab(t,s,a)}catch(e){return rm(e instanceof aE?"BloomFilter error: ":"Applying bloom filter failed: ",e),null}return 0===i.Ae?null:i}/**
...
n.Be(t),this.Ue=this.Ue.insert(t,this.ct(t).delete(e)),i&&(this.$e=this.$e.insert(t,i))}removeTarget(e){this.Ke.delete(e)}/**
     ...
     * specified target.
     */ut(e,t){return this.Qe.getRemoteKeysForTarget(e).has(t)}}function aN(){return new rG(rF.comparator)}function ak(){return new rG(rF.comparator)}const aL={asc:"ASCENDING",desc:"DESCENDING"},aR={"<":"LESS_THAN","<=":"LESS_THAN_OR_EQUAL",">":"GREATER_THAN",">=":"GREATER_THAN_OR_EQUAL","==":"EQUAL","!=":"NOT_EQUAL","array-contains":"ARRAY_CONTAINS",in:"IN","not-in":"NOT_IN","array-contains-any":"ARRAY_CONTAINS_ANY"},ax={and:"AND",or:"OR"};/**
...
m.Vo()},0),m}}/** The Platform's 'document' implementation or null if not available. */function ob(){// `document` is not always available, e.g. in ReactNative and WebWorkers.
 ...
 */class o_{constructor(e,t,i,n,r,s,a,o){this._i=e,this.Uo=i,this.Wo=n,this.connection=r,this.authCredentialsProvider=s,this.appCheckCredentialsProvider=a,this.listener=o,this.state=0/* PersistentStreamState.Initial */,/**
         ...
         * close.
         */this.Go=0,this.zo=null,this.jo=null,this.stream=null,this.Ho=new oE(e,t)}/**
 ...
 */return function(e){let t=lE.get(e);t&&(rd("ComponentProvider","Removing Datastore"),lE.delete(e),t.terminate())}(this),Promise.resolve()}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
this.ru=!1,this.iu=!1,this.su=[],this.Ho=new oE(this,"async_queue_retry"/* TimerId.AsyncQueueRetry */),// operations. Meant to speed up recovery when we regain file system access
// after page comes into foreground.
this.ou=()=>{let e=ob();e&&rd("AsyncQueue","Visibility state changed to "+e.visibilityState),this.Ho.$o()};let e=ob();e&&"function"==typeof e.addEventListener&&e.addEventListener("visibilitychange",this.ou)}get isShuttingDown(){return this.eu}/**
     * Adds a new operation to the queue without waiting for it to complete (i.e.
     * we ignore the Promise result).
     */enqueueAndForget(e){// eslint-disable-next-line @typescript-eslint/no-floating-promises
this.enqueue(e)}enqueueAndForgetEvenWhileRestricted(e){this._u(),this.au(e)}enterRestrictedMode(e){if(!this.eu){this.eu=!0,this.iu=e||!1;let t=ob();t&&"function"==typeof t.removeEventListener&&t.removeEventListener("visibilitychange",this.ou)}}enqueue(e){if(this._u(),this.eu)return new Promise(()=>{});// Create a deferred Promise that we can return to the callee. This
 ...
 *
 * @public
 */function(e=eg){let t=ev.get(e);if(!t&&e===eg&&M())return eT();if(!t)throw eE.create("no-app"/* AppError.NO_APP */,{appName:e});return t}(),n=/**
 *
...
iss:`https://securetoken.google.com/${i}`,aud:i,iat:n,exp:n+3600,auth_time:n,sub:r,user_id:r,firebase:{sign_in_provider:"custom",identities:{}}},e);return[D(JSON.stringify({alg:"none",type:"JWT"})),D(JSON.stringify(s)),""].join(".")}(n.mockUserToken,null===(r=e._app)||void 0===r?void 0:r.options.projectId);let s=n.mockUserToken.sub||n.mockUserToken.user_id;if(!s)throw new rv(ry.INVALID_ARGUMENT,"mockUserToken must contain 'sub' or 'user_id' field!");i=new rl(s)}e._authCredentials=new r_(new rb(t,i))}}(n,...e)}return n}(lH);console.log(lH),console.log(lG);class lQ{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, AdminFirebase is connected!")};get=async e=>{let t=await /**
...
//     }
// });
const hh=new class{constructor(){this.adminView=new h,this.adminModel=new u,this.adminFirebase=new lQ}initializeAppMain=async()=>{console.log("HELLO! INIT CARRIED OUT SUCCESFULLY"),this.adminView.checkModuleLinkage(),this.adminModel.checkModuleLinkage(),this.adminFirebase.checkModuleLinkage(),hn=await this.adminFirebase.get(l8),hr=await this.adminFirebase.get(l3),hs=await this.adminFirebase.get(hi),ha=await this.adminFirebase.get(he),ho=await this.adminFirebase.get(ht),hl=await this.adminFirebase.get(l7),// For TEST only:
// console.log("usersData: ", usersData);
// console.log("usersDataDb: ", usersDataDb);
// console.log("usersDataDb: ", usersDataDb[0]);
// console.log("usersDataDb: ", usersDataDb[1]);
this.displayTables(),this.adminView.renderMenu(l1,l2,l9,l4,l6,l5),this.attachEventListeners()};displayTables=async()=>{this.adminView.clearContainerLeft(),this.adminView.renderTable(lW,l9),this.adminView.renderTable(hn,l8),this.adminView.renderTable(lY,l1),this.adminView.renderTable(hr,l3),this.adminView.renderTable(lJ,l5),this.adminView.renderTable(hs,hi),this.adminView.renderTable(lZ,l4),this.adminView.renderTable(ha,he),this.adminView.renderTable(l0,l6),this.adminView.renderTable(ho,ht),this.adminView.renderTable(lX,l2),this.adminView.renderTable(hl,l7)};// Functions to split elemnt ID (${arrayName}_${object.docId}_${key})
...
getArrayByName(e){return"usersData"===e?lW:"usersDataDb"===e?hn:"customersData"===e?lY:"customersDataDb"===e?hr:"cartsData"===e?lJ:"cartsDataDb"===e?hs:"ordersData"===e?lZ:"ordersDataDb"===e?ha:"orderItemsData"===e?l0:"orderItemsDataDb"===e?ho:"productItems"===e?lX:"productItemsDb"===e?hl:null}// Add duplicate document (element/object)
...
n.docId=this.adminModel.generateNewId(12),n)}handleAddBtnClick=e=>{let t=this.getCollectionFromId(e.id),i=this.getDocFromId(e.id);console.log(`Request to clone element in ${t}, docId: ${i}.`),console.log("elementCollection =",t);let n=this.getArrayByName(t);console.log("affectedArray =",n);let r=this.duplicateDocument(n,i);console.log("clonedObject: ",r),// Add cloned object/document to array/collection:
...
;try{hh.initializeAppMain()}catch(e){console.error("An error occured duting app initialization:",e),alert("Something went wrong. Please contact the app admin or try again later.")}//# sourceMappingURL=index.93bca3e5.js.map

//# sourceMappingURL=index.93bca3e5.js.map
