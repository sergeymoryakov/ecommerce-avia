...
return(""+e[2]).replace(/^((?:https?|file|ftp|(chrome|moz|safari-web)-extension):\/\/.+)\/[^/]+$/,"$1")+"/"}return"/"}(),c[e]=t),t}}),i("4G3qk").register(JSON.parse('{"ceDhw":"index.31fb6938.js","4M8Gf":"icon-add-order.d5dcde0a.png","kSZwU":"icon-cart.dd455062.png","8HtV0":"icon-order.5ea58f4f.png","baUMc":"admin.7eece7fc.js"}'));var o={};o=i("260Jh").getBundleURL("ceDhw")+i("4G3qk").resolve("4M8Gf");var d={};d=i("260Jh").getBundleURL("ceDhw")+i("4G3qk").resolve("kSZwU");class a{constructor(e){this.controller=e}// FOR TEST AND TBS - REMOVE FOR PROD
...
s.appendChild(m),a.appendChild(l),a.appendChild(s),c.appendChild(i),c.appendChild(o),c.appendChild(a),r.appendChild(n),r.appendChild(c),r};// Left Container - Render product items
...
i.appendChild(d),l.appendChild(s),l.appendChild(m),l.appendChild(u),l.appendChild(p),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(_),l.appendChild(E),l.appendChild(I),f.appendChild(y),f.appendChild(k),a.appendChild(l),a.appendChild(f),c.appendChild(i),c.appendChild(a),v.appendChild(w),v.appendChild(b),r.appendChild(n),r.appendChild(c),r.appendChild(v),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
...
let r=this.createCartSummaryTitle(),n=this.createCartProductItemsBlock(e),c=this.createTotalAmountBlock(t),i=this.createProceedToCheckoutBtn();// TEST-TBS - REMOVE FOR PROD
// console.log("cartSummaryBlock: ", cartSummaryBlock);
...
r.id=`cart-item_${e.itemId}`;let n=document.createElement("div");n.classList="cart-item__img-wrapper";let c=document.createElement("img");c.classList="cart-item__img",c.src=this.controller.getUrlByNameLocal(e.itemImg);let i=document.createElement("div");i.classList="cart-item__summary-wrapper";let o=document.createElement("div");o.classList="cart-item__summary";let d=document.createElement("div");d.classList="cart-item__element-wrapper";let a=document.createElement("h1");a.classList="cart-item__product-name",a.textContent=e.itemName;let l=document.createElement("div");l.classList="cart-item__element-wrapper";let s=document.createElement("p");s.classList="cart-item__support-txt",s.textContent="Part Number: ";let m=document.createElement("h3");m.classList="cart-item__part-number",m.textContent=e.itemPN;let u=document.createElement("div");u.classList="cart-item__element-wrapper";let p=document.createElement("p");p.classList="cart-item__support-txt",p.textContent="Condition: ";let h=document.createElement("h3");h.classList="cart-item__condition",h.textContent=e.itemCondition;let C=document.createElement("div");C.classList="cart-item__element-wrapper";let g=document.createElement("p");g.classList="cart-item__support-txt",g.textContent="Certification: ";let L=document.createElement("h3");L.classList="cart-item__certification",L.textContent=e.itemCertification;let _=document.createElement("div");_.classList="cart-item__element-wrapper";let E=document.createElement("p");E.classList="cart-item__support-txt",E.textContent="Availability: ";let I=document.createElement("h3");I.classList="cart-item__availability",I.textContent=e.itemLeadTime;let f=document.createElement("div");f.classList="cart-item__price-qty";let y=document.createElement("h3");y.classList="cart-item__product-price",y.textContent=`${e.itemCurrency} ${e.itemPrice} x ${t}`;let k=document.createElement("div");k.classList="cart-element__qty-mod_wrapper",k.id=`qty-mod_${e.itemId}`;let v=document.createElement("button");v.classList="cart-element__qty-mod_deduct-btn",v.innerText=" - ",v.id=`qty-deduct-btn_${e.itemId}`;let w=document.createElement("button");w.classList="cart-element__qty-mod_qty",w.innerText=t;let b=document.createElement("button");// console.log("productCardMain: ", productCardMain);
return b.classList="cart-element__qty-mod_add-btn",b.innerText=" + ",b.id=`qty-add-btn_${e.itemId}`,// Assembly HTML code:
n.appendChild(c),d.appendChild(a),o.appendChild(d),l.appendChild(s),l.appendChild(m),o.appendChild(l),u.appendChild(p),u.appendChild(h),o.appendChild(u),C.appendChild(g),C.appendChild(L),o.appendChild(C),_.appendChild(E),_.appendChild(I),o.appendChild(_),k.appendChild(v),k.appendChild(w),k.appendChild(b),f.appendChild(y),f.appendChild(k),o.appendChild(f),i.appendChild(o),r.appendChild(n),r.appendChild(i),r};// To-DO items:
...
c.appendChild(this.createProductCardForCart(e,t.qty))}return t.appendChild(r),t.appendChild(n),t.appendChild(c),t}}class m{constructor(e){this.controller=e}checkModuleLinkage=()=>{console.log("HELLO, ViewOrders is connected!")};// Right Container - Create Orders History Title
...
return t.appendChild(r),t}}class u{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ViewAdmin is connected!")}}class p{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ViewSuperadmin is connected!")}}var h=i("9zRuU");class C{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelProducts is connected!")}}class g{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelCart is connected!")};getPriceByProductId=(e,t)=>{let r=e.filter(e=>e.itemId===t);// console.log("productById: ", productById);
// console.log("productById[0]: ", productById[0]);
return r[0].itemPrice};updateCartPriceVariable=(e,t,r,n)=>{let c=0,i=0;for(let t of e)c+=t.qty*this.getPriceByProductId(r,t.itemId);return i=c*n,t.items=c,t.handling=i,t.total=c+i,console.log("new sessionIdCartPrice: ",t),t};changeQtyInCart=(e,t,r)=>{for(let n of e)n.itemId===t&&(n.qty+r<=0?(confirm("Hi there, do you like to delete item from cart?"),console.log(`ACTION NEEDED: DELETE PRODUCT WITH ID ${n.itemId} FROM CART.`)):n.qty+=r);return e}}class L{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelOrders is connected!")}}class _{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelUser is connected!")};getSessionIdCustomerByCustomerId=(e,t)=>{let r=t.filter(t=>t.custId===e);return r[0]}}var E=i("ljOiA");// Init database instance (dataBase):
let I={},f="",y={},k={},v={},w={},b={items:8e5,handling:8e3,total:808e3},P=document.createElement("div");P.innerHTML="";let B=document.createElement("div");B.innerHTML="";let x=document.createElement("div");x.innerHTML="";// Init cross-ref Map: image - url
let T=new Map,O=new class{constructor(){this.viewProducts=new a(this),this.viewCart=new s(this),this.viewOrders=new m(this),this.viewAdmin=new u,this.viewSuperadmin=new p,this.modelFirebase=new h.ModelFirebase,this.modelProducts=new C,this.modelCart=new g,this.modelOrders=new L,this.modelUser=new _,this.containerLeftNode=document.getElementById("containerLeft"),this.containerRightNode=document.getElementById("containerRight")}initializeAppMain=async()=>{for(let e of(// TEST AND TBS - REMOVE IN PRODUCTION
...
T.set(e.itemImg,t)}console.log("Assigned to [*** sessionIdNumber ***]: ",// FOR TEST AND TBS - REMOVE IN PROD:
// console.log("imageLinksMap: ", imageLinksMap);
// Assign to sessionIdNumber a first ID from DB (usersData[0])
f=I.usersData[0].userId),console.log("[*** sessionIdUser ***] initiated: ",// Assign to sessionIdUser a first element of the usersData[0]
y=I.usersData[0]),// Get sessionIdCustomer data by customer's ID
this.getSessionIdCustomerData(),console.log("[*** sessionIdCustomer ***] initiated: ",k),// Alert - Popup inform the test name usage
...
w=this.getCartItemsByUserID(f),console.log(`Cart items [*** sessionIdCart ***] for User ID ${f}: `,w),// 2. Get orders list by user ID:
v=this.getOrdersByUserID(f),console.log(`Orders History [*** sessionIdOrders ***] for User ID ${f}: `,v),// Right Container - Render Cart Summary
this.renderCartAndOrdersSummary(w,v),this.attachEventListenrs()};// Get sessionIdCustomer data by customer's ID
getSessionIdCustomerData=()=>{k=this.modelUser.getSessionIdCustomerByCustomerId(y.custId,I.customersData)};// Clear container
...
handleGotoProductsBtn=()=>{this.renderProductItemsList(I.productItems),this.renderCartAndOrdersSummary(w,v)};// "Add to Cart" button at detailed product card (page)
...
this.modelCart.updateCartPriceVariable(w,b,I.productItems,k.custHandlingFee),this.renderCartSummary(w),this.renderCartSummaryWithPrice(w,b)};// "Proceed to checkout" button at right panel
...
console.log("Received command to display PRODUCT CARD with ID: ",e.id),console.log("Clicked inside product item");let t=this.getProdIdFromElementId(e.id),r=this.getProductObjectById(t);this.renderProductCard(r),this.renderCartAndOrdersSummary(w,v)};handleDisplayHistoricalOrder=e=>{console.log("Received command to display ORDER with ID: ",e.id)};handleQtyDeductBtn=(e,t)=>{console.log("Received command to deduct qty of product with ID: ",e.id);let r=this.getProdIdFromElementId(e.id);console.log("itemId: ",r),console.log("sessionIdCard: ",this.modelCart.changeQtyInCart(w,r,-1)),this.handleGotoCartBtn()};handleQtyAddBtn=(e,t)=>{console.log("Received command to add qty of product with ID: ",e.id);let r=this.getProdIdFromElementId(e.id);console.log("itemId: ",r),console.log("sessionIdCard: ",this.modelCart.changeQtyInCart(w,r,1)),this.handleGotoCartBtn()};handleButtonsClickLeft=(e,t)=>{let r=e.target;// "< Back to Products" button
...
let c=this.findAncestorByClass(r,"product-item__content_price-btn");if(c){this.handlePriceBtn(c);return}// Product card as "button" at main page
let i=this.findAncestorByClass(r,"product-item");// Qty deduct button at Cart page
if(i&&this.handleProductItem(i),r.classList.contains("cart-element__qty-mod_deduct-btn")){this.handleQtyDeductBtn(r,t);return}// Qty add button at Cart page
