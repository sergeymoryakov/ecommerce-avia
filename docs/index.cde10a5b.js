...
return(""+e[2]).replace(/^((?:https?|file|ftp|(chrome|moz|safari-web)-extension):\/\/.+)\/[^/]+$/,"$1")+"/"}return"/"}(),o[e]=t),t}}),i("4G3qk").register(JSON.parse('{"ceDhw":"index.cde10a5b.js","4M8Gf":"icon-add-order.d5dcde0a.png","kSZwU":"icon-cart.dd455062.png","8HtV0":"icon-order.5ea58f4f.png","kVJvh":"admin.4d7b58db.js"}'));var c={};c=i("260Jh").getBundleURL("ceDhw")+i("4G3qk").resolve("4M8Gf");var a={};a=i("260Jh").getBundleURL("ceDhw")+i("4G3qk").resolve("kSZwU");class d{constructor(e){this.controller=e}// FOR TEST AND TBS - REMOVE FOR PROD
checkModuleLinkage=()=>{console.log("HELLO, ViewProducts is connected!")};createProductItemCard=e=>{// Create elements
let r=document.createElement("div");r.classList="product-item",r.id=`product-card_${e.itemId}`;let n=document.createElement("img");n.classList="product-item__img",n.src=this.controller.getUrlByNameLocal(e.itemImg);let o=document.createElement("div");o.classList="product-item__content";let i=document.createElement("h3");i.classList="product-item__content_desc",i.textContent=e.itemName;let c=document.createElement("p");c.classList="product-item__content_part-number",c.textContent=e.itemPN;let d=document.createElement("div");d.classList="product-item__content_price-block";let l=document.createElement("h3");l.classList="product-item__content_price",l.textContent=`$${e.itemPrice}`;let s=document.createElement("button");s.classList="product-item__content_price-btn",s.id=`product-btn_${e.itemId}`;let m=document.createElement("img");return m.classList="product-item__content_price-btn-img",m.src=/*@__PURE__*/t(a),m.alt="Select",// Append elemens to respective parents
s.appendChild(m),d.appendChild(l),d.appendChild(s),o.appendChild(i),o.appendChild(c),o.appendChild(d),r.appendChild(n),r.appendChild(o),r};// Left Container - Render product items
...
i.appendChild(a),l.appendChild(s),l.appendChild(m),l.appendChild(u),l.appendChild(p),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(_),l.appendChild(I),l.appendChild(E),y.appendChild(f),y.appendChild(w),d.appendChild(l),d.appendChild(y),o.appendChild(i),o.appendChild(d),b.appendChild(P),b.appendChild(k),r.appendChild(n),r.appendChild(o),r.appendChild(b),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
...
let n={docId:this.controller.handleNewIdGeneration(),itemId:t,qty:1,userId:r};return console.log("a newCartElement: ",n),e.push(n),console.log("an updated [*** sessionIdCart ***]: ",e),e}}class L{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelOrders is connected!")}}class _{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelUser is connected!")};getSessionIdCustomerByCustomerId=(e,t)=>{let r=t.filter(t=>t.custId===e);return r[0]}}var I=i("ljOiA");// Init database instance (dataBase):
let E={},y="",f={},w={},b=[],P=[],k={items:8e5,handling:8e3,total:808e3},v=document.createElement("div");v.innerHTML="";let T=document.createElement("div");T.innerHTML="";let x=document.createElement("div");x.innerHTML="";// Init cross-ref Map: image - url
...
this.renderCartAndOrdersSummary(P,b),this.attachEventListenrs()};// Generate unique ID
handleNewIdGeneration=()=>this.modelFirebase.generateUniqueId();// Get sessionIdCustomer data by customer's ID
getSessionIdCustomerData=()=>{w=this.modelUser.getSessionIdCustomerByCustomerId(f.custId,E.customersData)};// Clear container
...
handleGotoProductsBtn=()=>{this.renderProductItemsList(E.productItems),this.renderCartAndOrdersSummary(P,b)};// "Add to Cart" button at detailed product card (page)
handleAddToCartBtn=e=>{let t=this.getProdIdFromElementId(e.id);console.log(`TO-DO: Add product with ID ${t} to cart.`),P=this.modelCart.addProductToCart(P,t,y),this.renderCartAndOrdersSummary(P,b)};// "Go to Cart" button at right panel
handleGotoCartBtn=()=>{console.log("Received command to display CART."),// TEST-TBD
this.modelCart.updateCartPriceVariable(P,k,E.productItems,w.custHandlingFee),this.renderCartSummary(P),this.renderCartSummaryWithPrice(P,k)};// "Proceed to checkout" button at right panel
...
console.log("Display CARD for product ID: ",e.id);let t=this.getProdIdFromElementId(e.id),r=this.getProductObjectById(t);this.renderProductCard(r),this.renderCartAndOrdersSummary(P,b)};handleDisplayHistoricalOrder=e=>{// TEST-TBS REMOVE IN PROD
...
console.log("sessionIdCart: ",t=this.modelCart.changeQtyInCart(t,n,r)),k=this.modelCart.updateCartPriceVariable(t,k,E.productItems,w.custHandlingFee)};handleViewOfPartQty=(e,t)=>{// TEST-TBS REMOVE IN PROD
