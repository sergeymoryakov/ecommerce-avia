 ...
 */const A=function(e){let t=T(e);return C.encodeByteArray(t,!0)},D=function(e){// Use base64url encoding and remove padding in the end (dot characters).
...
t.push(...i.split("/").filter(e=>e.length>0))}return new rO(t)}static emptyPath(){return new rO([])}}const rM=/^[_a-zA-Z][_a-zA-Z0-9]*$/;/**
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const aA=new ro([4294967295,4294967295],0);// Hash a string using md5 hashing algorithm.
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const ox="WebChannelConnection";class oP extends /**
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
get(e){if(this._document){let t=this._document.data.field(uA("DocumentSnapshot.get",e));if(null!==t)return this._userDataWriter.convertValue(t)}}}/**
...
let t=new uL(this._firestore,this._userDataWriter,this._key,this._document,this.metadata,/* converter= */null);return this._converter.fromFirestore(t,e)}return this._userDataWriter.convertValue(this._document.data.value,e.serverTimestamps)}}/**
     ...
     */// We are using `any` here to avoid an explicit cast by our users.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
get(e,t={}){if(this._document){let i=this._document.data.field(uA("DocumentSnapshot.get",e));if(null!==i)return this._userDataWriter.convertValue(i,t.serverTimestamps)}}}/**
 * A `QueryDocumentSnapshot` contains data read from a document in your
 * Firestore database as part of a query. The document is guaranteed to exist
 * and its data can be extracted with `.data()` or `.get(<field>)` to get a
 * specific field.
...
iss:`https://securetoken.google.com/${i}`,aud:i,iat:n,exp:n+3600,auth_time:n,sub:r,user_id:r,firebase:{sign_in_provider:"custom",identities:{}}},e);return[D(JSON.stringify({alg:"none",type:"JWT"})),D(JSON.stringify(s)),""].join(".")}(n.mockUserToken,null===(r=e._app)||void 0===r?void 0:r.options.projectId);let s=n.mockUserToken.sub||n.mockUserToken.user_id;if(!s)throw new rw(rv.INVALID_ARGUMENT,"mockUserToken must contain 'sub' or 'user_id' field!");i=new ru(s)}e._authCredentials=new rT(new rE(t,i))}}(n,...e)}return n}(uO);console.log(uO),console.log(uM);class uF{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, AdminFirebase is connected!")};get=async e=>{let t=await /**
...
// performing validation.
n=$(n);let l=r.mu(o);if(n instanceof up)s.push(o);else{let e=ug(n,l);null!=e&&(s.push(o),a.set(o,e))}});let o=new r0(s);return new uh(a,o,r.fieldTransforms)}(s,"updateDoc",e._key,t)).toMutation(e._key,au.exists(!0))])}(i,t),console.log("Item/document succesfully updated in Firestore, docId: ",t.docId)}catch(e){console.error("Error updating document in Firestore: ",e)}};deleteDocFromFirestore=async(e,t)=>{try{var i;await (i=l8(uM,e,t),uP(l9(i.firestore,ut),[new ab(i._key,au.none())]))}catch(e){console.error("Error deleting document: ",e)}}}// Preliminary Specification for Aviation Parts Marketplace
...
//     }
// });
const u7=new class{constructor(){this.adminView=new u,this.adminModel=new h,this.adminFirebase=new uF}initializeAppMain=async()=>{console.log("HELLO! INIT CARRIED OUT SUCCESFULLY"),this.adminView.checkModuleLinkage(),this.adminModel.checkModuleLinkage(),this.adminFirebase.checkModuleLinkage(),u2=await this.adminFirebase.get(uJ),u9=await this.adminFirebase.get(uY),u4=await this.adminFirebase.get(u1),u6=await this.adminFirebase.get(uZ),u5=await this.adminFirebase.get(u0),u3=await this.adminFirebase.get(uX),// For TEST only:
// console.log("usersData: ", usersData);
// console.log("usersDataDb: ", usersDataDb);
// console.log("usersDataDb: ", usersDataDb[0]);
// console.log("usersDataDb: ", usersDataDb[1]);
this.displayTables(),this.adminView.renderMenu(uz,uK,uH,uG,uQ,uW),this.attachEventListeners()};displayTables=async()=>{this.adminView.clearContainerLeft(),this.adminView.renderTable(uV,uH),this.adminView.renderTable(u2,uJ),this.adminView.renderTable(uU,uz),this.adminView.renderTable(u9,uY),this.adminView.renderTable(uj,uW),this.adminView.renderTable(u4,u1),this.adminView.renderTable(uq,uG),this.adminView.renderTable(u6,uZ),this.adminView.renderTable(u$,uQ),this.adminView.renderTable(u5,u0),this.adminView.renderTable(uB,uK),this.adminView.renderTable(u3,uX)};// Functions to split elemnt ID (${arrayName}_${object.docId}_${key})
...
getArrayByName(e){return"usersData"===e?uV:"usersDataDb"===e?u2:"customersData"===e?uU:"customersDataDb"===e?u9:"cartsData"===e?uj:"cartsDataDb"===e?u4:"ordersData"===e?uq:"ordersDataDb"===e?u6:"orderItemsData"===e?u$:"orderItemsDataDb"===e?u5:"productItems"===e?uB:"productItemsDb"===e?u3:null}// SUPPORT FUNCTION: GET SISTER ARRAY BY NAME:
getSisterArrayByName(e){return"usersData"===e?u2:"customersData"===e?u9:"cartsData"===e?u4:"ordersData"===e?u6:"orderItemsData"===e?u5:"productItems"===e?u3:null}// SUPPORT FUNCTION: GET SISTER ARRAY BY NAME:
...
;try{u7.initializeAppMain()}catch(e){console.error("An error occured duting app initialization:",e),alert("Something went wrong. Please contact the app admin or try again later.")}//# sourceMappingURL=index.62883cb1.js.map

//# sourceMappingURL=index.62883cb1.js.map
