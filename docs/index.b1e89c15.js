...
a.appendChild(d),l.appendChild(s),l.appendChild(m),l.appendChild(u),l.appendChild(p),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(_),l.appendChild(I),l.appendChild(E),y.appendChild(f),y.appendChild(P),c.appendChild(l),c.appendChild(y),o.appendChild(a),o.appendChild(c),b.appendChild(w),b.appendChild(v),r.appendChild(n),r.appendChild(o),r.appendChild(b),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
...
o.appendChild(this.createProductCardForCart(e,t.qty))}return t.appendChild(r),t.appendChild(n),t.appendChild(o),t};// Left Container - Create Empty Cart Page
...
return t.appendChild(r),t}}class u{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ViewAdmin is connected!")}}class p{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ViewSuperadmin is connected!")}}var h=a("keq33");class C{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelProducts is connected!")}}class g{constructor(e){this.controller=e}checkModuleLinkage=()=>{console.log("HELLO, ModelCart is connected!")}}class L{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelOrders is connected!")}}class _{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelUser is connected!")};getSessionIdCustomerByCustomerId=(e,t)=>{let r=t.filter(t=>t.custId===e);return r[0]}}var I=a("1ggDV");// Init database instance (dataBase):
let E={},y="",f={},P={},b=[],w=[],v={};// Init HTML variables:
...
b=this.getOrdersByUserID(y),console.log(`Orders History [*** sessionIdOrders ***] for User ID ${y}: `,b),// ALERT - POPOUP DISCLAMER
...
getSessionIdCustomerData=()=>P=this.modelUser.getSessionIdCustomerByCustomerId(f.custId,E.customersData);// Revert back from map an image URL by image name
getUrlByNameLocal=e=>x.get(e);// Set "sessionIdCart" by sesstion's userId (sessionIdNumber):
...
this.updateCartPriceVariable(w,v,E.productItems,P.custHandlingFee)};// MIGHT BE A PART OF MODEL:
updateCartPriceVariable=(e,t,r,n)=>{let o=0,a=0;for(let n of(// TO-DO: CHECK, if it is required here. Delete for now:
console.log("MODEL - [*** sessionIdCart ***]: ",e),console.log("MODEL - [*** sessionIdCartPrice ***]: ",t),console.log("MODEL - [*** dataBase.productItems ***]: ",E.productItems),console.log("MODEL - [*** sessionIdCustome ***]: ",P),e))o+=n.qty*this.getPriceByProductId(r,n.itemId);// TO-DO: CHECK, if it is required here. Delete for now:
// this.controller.handleViewOfTotalPrceInCart(sessionIdCartPrice.total);
return a=o*n,t.items=o,t.handling=a,t.total=o+a,console.log("new sessionIdCartPrice: ",t),console.log("for sessionIdCart: ",e),t};// Left Container - Render product items
...
renderCartSummary=e=>{let t;t=0===e.length?this.viewCart.createEmptyCartPage(e):this.viewCart.createCartPage(e),this.clearContainerLeft(),this.containerLeftNode.appendChild(t)};// Right Container - Render Cart (no price) and Orders Summaries
...
this.updateCartPriceVariable(w,v,E.productItems,P.custHandlingFee),this.renderCartSummary(w),this.renderCartSummaryWithPrice(w,v)};// "Proceed to checkout" button at right panel
...
let o=w.filter(e=>e.itemId!==r);n.qty+t>0?(this.updateProduct(o,n,t),this.updateCartDOM(r,n)):(this.deleteProduct(o,n),this.handleGotoCartBtn())};addProduct=(e,t)=>{let r={docId:this.handleNewIdGeneration(),itemId:e,qty:1,userId:t};console.log("MODEL: try to add to database.cartsData a [*** newProductToCart *** ]: ",r),this.handleAddDocToFirestore("cartsData",r),w.push(r),console.log("Updated [*** sessionIdCart ***]: ",w)};updateProduct=(e,t,r)=>{// Modify product/object
