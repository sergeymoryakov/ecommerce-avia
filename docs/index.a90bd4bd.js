...
a.appendChild(c),l.appendChild(s),l.appendChild(m),l.appendChild(u),l.appendChild(p),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(I),l.appendChild(_),l.appendChild(y),E.appendChild(f),E.appendChild(b),d.appendChild(l),d.appendChild(E),o.appendChild(a),o.appendChild(d),P.appendChild(w),P.appendChild(T),r.appendChild(n),r.appendChild(o),r.appendChild(P),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
// import iconCart from "../../other_assets/icon-cart.png";
// import iconCc from "../../other_assets/icon-cc.png";
var l={};l=new URL(a("d5gZm").resolve("5WNTT"),import.meta.url).toString();class s{constructor(e){this.controller=e}checkModuleLinkage=()=>{console.log("HELLO, ViewCart is connected!")};// Right Container - Create Cart Title
...
n.appendChild(o),c.appendChild(d),i.appendChild(c),l.appendChild(s),l.appendChild(m),i.appendChild(l),u.appendChild(p),u.appendChild(h),i.appendChild(u),C.appendChild(g),C.appendChild(L),i.appendChild(C),I.appendChild(_),I.appendChild(y),i.appendChild(I),b.appendChild(P),b.appendChild(w),b.appendChild(T),E.appendChild(f),E.appendChild(b),i.appendChild(E),a.appendChild(i),r.appendChild(n),r.appendChild(a),r};// To-DO items:
...
let n=this.controller.getDocIdFromArrayByItemId(e,t);if(n)this.changeQtyInCart(e,t,1);else{let n={docId:this.controller.handleNewIdGeneration(),itemId:t,qty:1,userId:r};console.log(`MODEL: try to add a [*** newCartElement *** ]: ${n} to database.cartsData`),this.controller.handleAddDocToFirestore("cartsData",n),e.push(n),console.log("an updated [*** sessionIdCart ***]: ",e)}return e}}class L{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelOrders is connected!")}}class I{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelUser is connected!")};getSessionIdCustomerByCustomerId=(e,t)=>{let r=t.filter(t=>t.custId===e);return r[0]}}var _=a("1ggDV");// Init database instance (dataBase):
let y={},E="",f={},b={},P=[],w=[],T={items:8e5,handling:8e3,total:808e3};// Init HTML variables:
const D=document.createElement("div");D.innerHTML="";const v=document.createElement("div");v.innerHTML="";const k=document.createElement("div");k.innerHTML="";// Init cross-ref Map: image - url
const O=new Map,B=new class{constructor(){this.viewProducts=new d(this),this.viewCart=new s(this),this.viewOrders=new m(this),this.viewAdmin=new u,this.viewSuperadmin=new p,this.modelFirebase=new h.ModelFirebase,this.modelProducts=new C,this.modelCart=new g(this),this.modelOrders=new L,this.modelUser=new I,this.containerLeftNode=document.getElementById("containerLeft"),this.containerRightNode=document.getElementById("containerRight")}initializeAppMain=async()=>{for(let e of(// TEST AND TBS - REMOVE IN PRODUCTION
...
console.log("Action: Getdata from database (Firebase) and keep in dataBase{}."),_.dbCollectionNames))y[e]=await this.modelFirebase.get(e),console.log(`[*** dataBase.${e} ***] :`,y[e]);for(let e of(// Create Image Links Map:
...
E=y.usersData[0].userId),console.log("[*** sessionIdUser ***] initiated: ",// Assign to sessionIdUser a first object/document from DB (usersData[0])
f=y.usersData[0]),console.log("[*** sessionIdCustomer ***] initiated: ",// Assign to sessionIdCustomer an object/document associated with sesstionIdUser (by userId)
...
w=this.getCartItemsByUserID(E),console.log(`Cart items [*** sessionIdCart ***] for User ID ${E}: `,w),// 2. Set "sessionIdOrders" by sesstion's userId (sessionIdNumber):
P=this.getOrdersByUserID(E),console.log(`Orders History [*** sessionIdOrders ***] for User ID ${E}: `,P),// ALERT - POPOUP DISCLAMER
...
getSessionIdCustomerData=()=>b=this.modelUser.getSessionIdCustomerByCustomerId(f.custId,y.customersData);// Revert back from map an image URL by image name
...
this.updateCartPriceVariable(w,T,y.productItems,b.custHandlingFee)};// MIGHT BE A PART OF MODEL:
updateCartPriceVariable=(e,t,r,n)=>{let o=0,a=0;for(let n of(// TO-DO: CHECK, if it is required here. Delete for now:
console.log("MODEL - [*** sessionIdCart ***]: ",e),console.log("MODEL - [*** sessionIdCartPrice ***]: ",t),console.log("MODEL - [*** dataBase.productItems ***]: ",y.productItems),console.log("MODEL - [*** sessionIdCustome ***]: ",b),e))o+=n.qty*this.modelCart.getPriceByProductId(r,n.itemId);// TO-DO: CHECK, if it is required here. Delete for now:
// this.controller.handleViewOfTotalPrceInCart(sessionIdCartPrice.total);
return a=o*n,t.items=o,t.handling=a,t.total=o+a,console.log("new sessionIdCartPrice: ",t),t};handleChangeQtyInCart=(e,t,r)=>{for(let n of(// TEST-TBS REMOVE IN PROD
console.log("sessionIdCart: ",e),e))if(n.itemId===t){if(n.qty+r>0)n.qty+=r,console.log("MODEL: try to update in sessionIdCart a [*** product *** ]: ",n),this.handleUpdateDocInFirestore("cartsData",n),// TO-DO: CHECK, if it is required here. Delete for now:
// console.log(
//     "MODEL - [*** sessionIdCart ***]: ",
//     sessionIdCart
// );
...
this.callToUpdateCartPriceVariable(),this.handleViewOfPartQty(t,n.qty);else{confirm("Hi there, do you like to delete item from cart?");// Get docId by itemId
let r=this.getDocIdFromArrayByItemId(e,t);console.log(`MODEL: TRY TO DELETE PRODUCT WITH DocID ${r} from database.cartsData`),this.handleDeleteDocFromFirestore("cartsData",r),// Delete object element with docId from array:
e=this.deleteObjectFromArrayByDocId(e,r),// TO-DO: CHECK, if it is required here. Delete for now:
// console.log(
//     "MODEL - [*** sessionIdCart ***]: ",
//     sessionIdCart
// );
...
//     sessionIdCustomer
// );
this.callToUpdateCartPriceVariable(),// TO-DO: CHECK, if it is required here. Delete for now:
this.renderCartSummary(e)}}return this.handleViewOfTotalPrceInCart(),e};// Left Container - Render product items
...
handleAddToCartBtn=e=>{let t=this.getProdIdFromElementId(e.id);console.log(`TO-DO: Add product with ID ${t} to cart.`),w=this.modelCart.addProductToCart(w,t,E),this.renderCartAndOrdersSummary(w,P)};// "Go to Cart" button at right panel
handleGotoCartBtn=()=>{console.log("Received command to display CART."),// TEST-TBD
this.updateCartPriceVariable(w,T,y.productItems,b.custHandlingFee),this.renderCartSummary(w),this.renderCartSummaryWithPrice(w,T)};// "Proceed to checkout" button at right panel
...
console.log(`Function: handleQtyChangeBtn() - request to update QTY of product ID: ${e.id}, for "sessionIdCart: ${t}`);let n=this.getProdIdFromElementId(e.id);// TEST-TBS REMOVE IN PROD
console.log("itemId: ",n),t=this.handleChangeQtyInCart(t,n,r,T,b);// TO-DO: CHECK, if it is required here. Delete for now:
...
if(o&&this.handleProductItem(o),t.classList.contains("cart-element__qty-mod_deduct-btn")){this.handleQtyChangeBtn(t,w,-1);return}// Qty add button at Cart page
if(t.classList.contains("cart-element__qty-mod_add-btn")){this.handleQtyChangeBtn(t,w,1);return}};handleButtonsClickRight=e=>{let t=e.target;t.classList.contains("cart-link-btn")&&this.handleProductItem(t),t.classList.contains("order-number-btn")&&this.handleDisplayHistoricalOrder(t);// "Go to cart" button at right panel
