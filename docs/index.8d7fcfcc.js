...
s.appendChild(a),l.appendChild(h),l.appendChild(c),l.appendChild(u),l.appendChild(d),l.appendChild(p),l.appendChild(f),l.appendChild(m),l.appendChild(g),l.appendChild(y),l.appendChild(w),l.appendChild(_),E.appendChild(C),E.appendChild(b),o.appendChild(l),o.appendChild(E),i.appendChild(s),i.appendChild(o),T.appendChild(I),T.appendChild(S),t.appendChild(r),t.appendChild(i),t.appendChild(T),t};createSpinner=()=>{let e=document.createElement("div");e.classList="spinnerFrame";let t=document.createElement("div");t.classList="spinner",t.id="spinner";let r=document.createElement("div");return r.classList="spinnerText",r.textContent="One moment, I'm downloading content...",e.appendChild(t),e.appendChild(r),e}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
 ...
 * provider.
 */function(e){let t=e.getComponent();return(null==t?void 0:t.type)==="VERSION"/* ComponentType.VERSION */}(e))return null;{let t=e.getImmediate();return`${t.library}/${t.version}`}}).filter(e=>e).join(" ")}}let ek="@firebase/app",eN="0.9.19",eL=new ef("@firebase/app"),eR="[DEFAULT]",eP={[ek]:"fire-core","@firebase/app-compat":"fire-core-compat","@firebase/analytics":"fire-analytics","@firebase/analytics-compat":"fire-analytics-compat","@firebase/app-check":"fire-app-check","@firebase/app-check-compat":"fire-app-check-compat","@firebase/auth":"fire-auth","@firebase/auth-compat":"fire-auth-compat","@firebase/database":"fire-rtdb","@firebase/database-compat":"fire-rtdb-compat","@firebase/functions":"fire-fn","@firebase/functions-compat":"fire-fn-compat","@firebase/installations":"fire-iid","@firebase/installations-compat":"fire-iid-compat","@firebase/messaging":"fire-fcm","@firebase/messaging-compat":"fire-fcm-compat","@firebase/performance":"fire-perf","@firebase/performance-compat":"fire-perf-compat","@firebase/remote-config":"fire-rc","@firebase/remote-config-compat":"fire-rc-compat","@firebase/storage":"fire-gcs","@firebase/storage-compat":"fire-gcs-compat","@firebase/firestore":"fire-fst","@firebase/firestore-compat":"fire-fst-compat","fire-js":"fire-js",firebase:"fire-js-all"},eO=new Map,ex=new Map;/**
 *
...
this.Ze(t),this.We=this.We.insert(t,2/* BloomFilterApplicationStatus.FalsePositive */===i?"TargetPurposeExistenceFilterMismatchBloom"/* TargetPurpose.ExistenceFilterMismatchBloom */:"TargetPurposeExistenceFilterMismatch"/* TargetPurpose.ExistenceFilterMismatch */))}}}}/**
     ...
     * filter.
     */nt(e){let t,r;let n=e.ye.unchangedNames;if(!n||!n.bits)return null;let{bits:{bitmap:i="",padding:s=0},hashCount:a=0}=n;try{t=sf(i).toUint8Array()}catch(e){if(e instanceof sh)return iP("Decoding the base64 bloom filter in existence filter failed ("+e.message+"); ignoring the bloom filter and falling back to full re-query."),null;throw e}try{// BloomFilter throws error if the inputs are invalid.
...
// already saved.
for(;a.hasNext();){let n=a.getNext(),o=n.key,l=n.value,h=ah();l.forEach(e=>{if(!i.has(e)){let n=aN(t.get(e),r.get(e));null!==n&&h.set(e,n),i=i.add(e)}}),s.push(this.documentOverlayCache.saveOverlays(e,o,h))}return i5.waitFor(s)}).next(()=>r)}/**
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let t=c[e];if(void 0!==t)return aH(t)}(t),i=s.message;void 0===r&&(r=iM.INTERNAL,i="Unknown error status: "+t+" with message "+s.message),p=!0,f.mo(new iF(r,i)),u.close()}else iL(oY,`RPC '${e}' stream ${n} received:`,i),f.fo(i)}}),m(a,i_.STAT_EVENT,t=>{t.stat===iE.PROXY?iL(oY,`RPC '${e}' stream ${n} detected buffering proxy`):t.stat===iE.NOPROXY&&iL(oY,`RPC '${e}' stream ${n} detected no buffering proxy`)}),setTimeout(()=>{// Technically we could/should wait for the WebChannel opened event,
...
let i=new of(n.target,t,r,n.sequenceNumber);lr(e,i)}),e.remoteSyncer.applyRemoteEvent(r)}(e,r)}catch(t){iL("RemoteStore","Failed to raise snapshot:",t),await lc(e,t)}}/**
...
// tokens.
this.authCredentials.shutdown(),this.appCheckCredentials.shutdown(),e.resolve()}catch(r){let t=lT(r,"Failed to shutdown persistence");e.reject(t)}}),e.promise}}async function he(e,t){e.asyncQueue.verifyOperationInProgress(),iL("FirestoreClient","Initializing OfflineComponentProvider");let r=await e.getConfiguration();await t.initialize(r);let n=r.initialUser;e.setCredentialChangeListener(async e=>{n.isEqual(e)||(await oB(t.localStore,e),n=e)}),// need to be terminated to allow the delete to succeed.
t.persistence.setDatabaseDeletedListener(()=>e.terminate()),e._offlineComponents=t}async function ht(e,t){e.asyncQueue.verifyOperationInProgress();let r=await hr(e);iL("FirestoreClient","Initializing OnlineComponentProvider");let n=await e.getConfiguration();await t.initialize(r,n),// precedence over the offline component provider.
 ...
 */return function(e){let t=ha.get(e);t&&(iL("ComponentProvider","Removing Datastore"),ha.delete(e),t.terminate())}(this),Promise.resolve()}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
case 1/* UserDataSource.Update */:return!0;case 3/* UserDataSource.Argument */:case 4/* UserDataSource.ArrayArgument */:return!1;default:throw ix()}}/** A "context" object passed around while parsing user data. */class hN{/**
 ...
 * classes.
 */class hL{constructor(e,t,r){this.databaseId=e,this.ignoreUndefinedProperties=t,this.serializer=r||o0(e)}/** Creates a new top-level parse context. */wu(e,t,r,n=!1){return new hN({Tu:e,methodName:t,yu:r,path:i1.emptyPath(),Ru:!1,pu:n},this.databaseId,this.serializer,this.ignoreUndefinedProperties)}}function hR(e){let t=e._freezeSettings(),r=o0(e._databaseId);return new hL(e._databaseId,!!t.ignoreUndefinedProperties,r)}class hP extends hT{_toFieldTransform(e){if(2/* UserDataSource.MergeSet */!==e.Tu)throw 1/* UserDataSource.Update */===e.Tu?e.gu(`${this._methodName}() can only appear at the top level of your update data`):e.gu(`${this._methodName}() cannot be used with set() unless you pass {merge:true}`);// No transform to add for a delete, but we need to add it to our
// fieldMask so it gets deleted.
return e.fieldMask.push(e.path),null}isEqual(e){return e instanceof hP}}/**
 ...
 */function hO(e,t){if(hM(// from firestore-exp.
...
if(t.settings.Ru&&4/* UserDataSource.ArrayArgument */!==t.Tu)throw t.gu("Nested arrays are not supported");return function(e,t){let r=[],n=0;for(let i of e){let e=hO(i,t.fu(n));null==e&&// Just include nulls in the array for fields being replaced with a
// sentinel.
 ...
 * Security Rules.
 */function(e,t,r,n={}){var i;let s=(e=hu(e,hp))._getSettings(),a=`${t}:${r}`;if("firestore.googleapis.com"!==s.host&&s.host!==a&&iP("Host has been set in both settings() and connectFirestoreEmulator(), emulator host will be used."),e._setSettings(Object.assign(Object.assign({},s),{host:a,ssl:!1})),n.mockUserToken){let t,r;if("string"==typeof n.mockUserToken)t=n.mockUserToken,r=iA.MOCK_USER;else{// Let createMockUserToken validate first (catches common mistakes like
...
r=ei(r);let n=a.mu(t);if(r instanceof hP)h.push(t);else{let e=hO(r,n);null!=e&&(h.push(t),c.set(t,e))}}let u=new sl(h);return new hD(c,u,a.fieldTransforms)}(s,"updateDoc",e._key,t,void 0,n):/** Parse update data from an update() call. */function(e,t,r,n){let i=e.wu(1/* UserDataSource.Update */,t,r);hF("Data must be an object, but it was:",i,n);let s=[],a=sO.empty();st(n,(e,n)=>{let o=hV(t,e,r);// For Compat types, we have to "extract" the underlying types before
// performing validation.
n=ei(n);let l=i.mu(o);if(n instanceof hP)s.push(o);else{let e=hO(n,l);null!=e&&(s.push(o),a.set(o,e))}});let o=new sl(s);return new hD(a,o,i.fieldTransforms)}(s,"updateDoc",e._key,t)).toMutation(e._key,aA.exists(!0))])}(r,t),console.log("Item/document succesfully updated in Firestore, docId: ",t.docId)}catch(e){console.error("Error updating document in Firestore: ",e)}};deleteDocFromFirestore=async(e,t)=>{try{var r;await (r=hy(cA,e,t),hJ(hu(r.firestore,hw),[new aU(r._key,aA.none())]))}catch(e){console.error("Error deleting document: ",e)}};// Support Functions
// Generate unique ID
...
await this.getUpdateLocalDataBase(),this.renderSpinner(),cB.productItems)){let t=await this.modelFirebase.getLinkToImage(e.itemImg).then(e=>e).catch(e=>{console.log("Error getting download URL: ",e)});// FOR TEST AND TBS - REMOVE IN PROD:
...
this.renderCartAndOrdersSummary(cH,cq),this.attachEventListenrs()};renderSpinner=()=>{let e=this.viewProducts.createSpinner();this.clearContainerLeft(),this.containerLeftNode.appendChild(e)};// TO-DO: check where it is used?
...
this.popupContentNode.innerHTML="";let e=this.viewPopup.createPopupMessage(cM);this.popupContentNode.appendChild(e),this.togglePopup()};activatePopupMessage=e=>{// Clear Popup Content
