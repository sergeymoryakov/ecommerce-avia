...
return(""+e[2]).replace(/^((?:https?|file|ftp|(chrome|moz|safari-web)-extension):\/\/.+)\/[^/]+$/,"$1")+"/"}return"/"}(),o[e]=t),t}}),c("4G3qk").register(JSON.parse('{"ceDhw":"index.8f5583c7.js","4M8Gf":"icon-add-order.d5dcde0a.png","kSZwU":"icon-cart.dd455062.png","8HtV0":"icon-order.5ea58f4f.png","kVJvh":"admin.20c8b860.js"}'));var a={};a=c("260Jh").getBundleURL("ceDhw")+c("4G3qk").resolve("4M8Gf");var i={};i=c("260Jh").getBundleURL("ceDhw")+c("4G3qk").resolve("kSZwU");class d{constructor(e){this.controller=e}// FOR TEST AND TBS - REMOVE FOR PROD
checkModuleLinkage=()=>{console.log("HELLO, ViewProducts is connected!")};createProductItemCard=e=>{// Create elements
let r=document.createElement("div");r.classList="product-item",r.id=`product-card_${e.itemId}`;let n=document.createElement("img");n.classList="product-item__img",n.src=this.controller.getUrlByNameLocal(e.itemImg);let o=document.createElement("div");o.classList="product-item__content";let c=document.createElement("h3");c.classList="product-item__content_desc",c.textContent=e.itemName;let a=document.createElement("p");a.classList="product-item__content_part-number",a.textContent=e.itemPN;let d=document.createElement("div");d.classList="product-item__content_price-block";let l=document.createElement("h3");l.classList="product-item__content_price",l.textContent=`$${e.itemPrice}`;let s=document.createElement("button");s.classList="product-item__content_price-btn",s.id=`product-btn_${e.itemId}`;let m=document.createElement("img");return m.classList="product-item__content_price-btn-img",m.src=/*@__PURE__*/t(i),m.alt="Select",// Append elemens to respective parents
s.appendChild(m),d.appendChild(l),d.appendChild(s),o.appendChild(c),o.appendChild(a),o.appendChild(d),r.appendChild(n),r.appendChild(o),r};// Left Container - Render product items
...
c.appendChild(i),l.appendChild(s),l.appendChild(m),l.appendChild(u),l.appendChild(p),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(_),l.appendChild(I),l.appendChild(E),y.appendChild(f),y.appendChild(b),d.appendChild(l),d.appendChild(y),o.appendChild(c),o.appendChild(d),w.appendChild(P),w.appendChild(v),r.appendChild(n),r.appendChild(o),r.appendChild(w),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
...
let r=this.createCartSummaryTitle(),n=this.createCartProductItemsBlock(e),o=this.createTotalAmountBlock(t),c=this.createProceedToCheckoutBtn();// TEST-TBS - REMOVE FOR PROD
// console.log("cartSummaryBlock: ", cartSummaryBlock);
return r.appendChild(n),r.appendChild(o),r.appendChild(c),r};// Left Container - Create Product Item Card block
...
P=this.getCartItemsByUserID(y),console.log(`Cart items [*** sessionIdCart ***] for User ID ${y}: `,P),// 2. Set "sessionIdOrders" by sesstion's userId (sessionIdNumber):
w=this.getOrdersByUserID(y),console.log(`Orders History [*** sessionIdOrders ***] for User ID ${y}: `,w),// ALERT - POPOUP DISCLAMER
...
this.updateCartPriceVariable(P,v,E.productItems,b.custHandlingFee)};// MIGHT BE A PART OF MODEL:
updateCartPriceVariable=(e,t,r,n)=>{let o=0,c=0;for(let n of(// TO-DO: CHECK, if it is required here. Delete for now:
...
// this.controller.handleViewOfTotalPrceInCart(sessionIdCartPrice.total);
return c=o*n,t.items=o,t.handling=c,t.total=o+c,console.log("new sessionIdCartPrice: ",t),console.log("for sessionIdCart: ",e),t};// handleChangeQtyInCart = (sessionIdCart, itemId, adder) => {
//     // TEST-TBS REMOVE IN PROD
//     console.log("sessionIdCart: ", sessionIdCart);
//     for (const product of sessionIdCart) {
//         if (product.itemId === itemId) {
...
this.renderCartAndOrdersSummary(P,w)};// "Add to Cart" button at detailed product card (page)
...
this.updateCartPriceVariable(P,v,E.productItems,b.custHandlingFee),this.renderCartSummary(P),this.renderCartSummaryWithPrice(P,v)};// "Proceed to checkout" button at right panel
...
console.log("Function: 'handleQtyChangeBtn()': update QTY of prod ID: ",e.id),console.log("'sessionIdCart': ",P),console.log("itemId: ",r);// product/object to update:
let n=JSON.parse(JSON.stringify(P.find(e=>e.itemId===r)));if(!n){console.log("Product/object not found");return}// remove product from array:
let o=P.filter(e=>e.itemId!==r);n.qty+t>0?this.updateProduct(o,n,t):this.deleteProduct(o,n),this.updateCartDOM();// // DELETE EVERYTHING BELOW:
...
P=e}deleteProduct(e,t){// Delete product from Firestore
this.handleDeleteDocFromFirestore("cartsData",t.docId),// Update global variable
P=e}updateCartDOM(){this.handleGotoCartBtn()}// Update product QTY in CART in 2 locations: price and selector
