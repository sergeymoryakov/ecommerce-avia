...
a.appendChild(d),l.appendChild(s),l.appendChild(m),l.appendChild(u),l.appendChild(p),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(_),l.appendChild(I),l.appendChild(y),E.appendChild(f),E.appendChild(b),c.appendChild(l),c.appendChild(E),o.appendChild(a),o.appendChild(c),w.appendChild(P),w.appendChild(T),r.appendChild(n),r.appendChild(o),r.appendChild(w),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
...
let n=this.controller.getDocIdFromArrayByItemId(e,t);if(n)this.changeQtyInCart(e,t,1);else{let n={docId:this.controller.handleNewIdGeneration(),itemId:t,qty:1,userId:r};console.log(`MODEL: try to add a [*** newCartElement *** ]: ${n} to database.cartsData`),this.controller.handleAddDocToFirestore("cartsData",n),e.push(n),console.log("an updated [*** sessionIdCart ***]: ",e)}return e}}class L{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelOrders is connected!")}}class _{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelUser is connected!")};getSessionIdCustomerByCustomerId=(e,t)=>{let r=t.filter(t=>t.custId===e);return r[0]}}var I=a("1ggDV");// Init database instance (dataBase):
let y={},E="",f={},b={},w=[],P=[],T={items:8e5,handling:8e3,total:808e3};// Init HTML variables:
...
this.modelCart.updateCartPriceVariable(P,T,y.productItems,b.custHandlingFee),this.renderCartSummary(P),this.renderCartSummaryWithPrice(P,T)};// "Proceed to checkout" button at right panel
...
console.log("sessionIdCart: ",t=this.modelCart.changeQtyInCart(t,n,r)),T=this.modelCart.updateCartPriceVariable(t,T,y.productItems,b.custHandlingFee)};handleViewOfPartQty=(e,t)=>{// TEST-TBS REMOVE IN PROD
