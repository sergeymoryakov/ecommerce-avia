...
o.appendChild(d),l.appendChild(s),l.appendChild(m),l.appendChild(u),l.appendChild(p),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(E),l.appendChild(I),l.appendChild(_),y.appendChild(f),y.appendChild(P),i.appendChild(l),i.appendChild(y),a.appendChild(o),a.appendChild(i),w.appendChild(b),w.appendChild(k),r.appendChild(n),r.appendChild(a),r.appendChild(w),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
...
createOrderSummaryPriceBlock=(e,t)=>{let r=document.createElement("div");r.classList="order-summary";let n=document.createElement("h3");n.classList="order-summary-title",n.innerText="Order Summary";let a=document.createElement("div");a.classList="flex-between";let o=document.createElement("p");o.innerText="Items:";let c=document.createElement("p");c.innerText=`$${e.items}`;let d=document.createElement("div");d.classList="flex-between";let i=document.createElement("p");i.innerText="Handling:";let l=document.createElement("p");l.innerText=`$${e.handling}`;let s=document.createElement("div");s.classList="flex-between bold";let m=document.createElement("p");m.innerText="Total:";let u=document.createElement("p");return u.innerText=`$${e.total}`,a.appendChild(o),a.appendChild(c),d.appendChild(i),d.appendChild(l),s.appendChild(m),s.appendChild(u),r.appendChild(n),r.appendChild(a),r.appendChild(d),r.appendChild(s),t&&r.appendChild(this.createPlaceOrderButtonBlock()),r};// Right Container - Create PlaceOrderButtonBlock
...
createBillToAddressContentBlock=(e,t)=>{let r=document.createElement("div");r.classList="address-wrapper";let n=document.createElement("h3");n.classList="address-title",n.innerText="Bill to Address";let a=document.createElement("p");a.innerText=e.custLegalName;let o=document.createElement("p");o.innerText=e.custBillToAddress;let c=document.createElement("p");c.innerText=e.userName;let d=document.createElement("p");d.innerText=e.userPhone;let i=document.createElement("p");return i.innerText=e.userEmail,r.appendChild(n),r.appendChild(a),r.appendChild(o),r.appendChild(c),r.appendChild(d),r.appendChild(i),t&&r.appendChild(this.createUpdateButtonBlock()),r};// Left Container - Create PaymentMethodContentBlock
...
getUpdateLocalDataBase=async()=>{for(let e of(console.log("Get or Update data from Firebase and keep in local dataBase{}."),I.dbCollectionNames))_[e]=await this.modelFirebase.get(e),console.log(`[*** dataBase.${e} ***] :`,_[e])};deleteObjectFromArrayByDocId=(e,t)=>{let r=e.filter(e=>e.docId===t);return -1!==r?(e.splice(r,1),console.log("Deleted element/document with docId = ",t)):console.log("Not found element/document with docId = ",t),e};// Assign to sessionIdCustomer an object/document associated with sesstionIdUser (by userId)
...
return o=a*n,t.items=a,t.handling=o,t.total=a+o,console.log("new sessionIdCartPrice: ",t),console.log("for sessionIdCartItems: ",e),t};// Left Container - Render product items
...
let t=this.getProdIdFromElementId(e.id),r=this.getDocIdFromArrayByItemId(b,t);r?this.handleQtyChangeBtn(e,1):this.addProduct(t,y),this.renderCartAndOrdersSummary(b,w)};// "Go to Cart" button at right panel or checkout panel
handleGotoCartBtn=()=>{// TEST-TBS REMOVE IN PROD
console.log("Received command to display CART."),this.updateCartPriceVariable(b,k,_.productItems,P.custHandlingFee),this.renderCartSummary(b),this.renderCartSummaryWithPrice(b,k)};// Handle Order Placement
...
let e=JSON.parse(JSON.stringify(this.modelFirebase.setNewOrderDetails(v,k)));// TEST-TBS REMOVE IN PROD
...
let t=JSON.parse(JSON.stringify(b));for(let r in // TEST-TBS REMOVE IN PROD
...
for(let e in b){console.log("[*** sessionIdCartItems[cartItem].docId ***] to delete from cartsData: ",b[e].docId);try{this.handleDeleteDocFromFirestore("cartsData",b[e].docId)}catch(e){console.error("Error deleting sessionIdCartItems from Firestore: ",e)}}console.log("UPDATE LOCAL DATABASE VARIABLES HERE");try{await this.getUpdateLocalDataBase()}catch(e){console.error("Error updating local variables: ",e)}};// "Proceed to checkout" button at right panel
