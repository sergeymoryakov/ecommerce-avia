...
c.appendChild(o),l.appendChild(s),l.appendChild(m),l.appendChild(u),l.appendChild(p),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(_),l.appendChild(E),l.appendChild(I),y.appendChild(f),y.appendChild(w),d.appendChild(l),d.appendChild(y),i.appendChild(c),i.appendChild(d),P.appendChild(v),P.appendChild(b),r.appendChild(n),r.appendChild(i),r.appendChild(P),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
...
let r=this.createCartSummaryTitle(),n=this.createCartProductItemsBlock(e),i=this.createTotalAmountBlock(t),c=this.createProceedToCheckoutBtn();// TEST-TBS - REMOVE FOR PROD
// console.log("cartSummaryBlock: ", cartSummaryBlock);
return r.appendChild(n),r.appendChild(i),r.appendChild(c),r};// Left Container - Create Product Item Card block
...
let n={docId:this.controller.handleNewIdGeneration(),itemId:t,qty:1,userId:r};return console.log("a newCartElement: ",n),e.push(n),console.log("an updated [*** sessionIdCart ***]: ",e),e}}class L{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelOrders is connected!")}}class _{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelUser is connected!")};getSessionIdCustomerByCustomerId=(e,t)=>{let r=t.filter(t=>t.custId===e);return r[0]}}var E=c("ljOiA");// Init database instance (dataBase):
let I={},y="",f={},w={},P={},v={},b={items:8e5,handling:8e3,total:808e3},k=document.createElement("div");k.innerHTML="";let T=document.createElement("div");T.innerHTML="";let x=document.createElement("div");x.innerHTML="";// Init cross-ref Map: image - url
...
this.renderCartAndOrdersSummary(v,P),this.attachEventListenrs()};// Generate unique ID
...
handleGotoProductsBtn=()=>{this.renderProductItemsList(I.productItems),this.renderCartAndOrdersSummary(v,P)};// "Add to Cart" button at detailed product card (page)
handleAddToCartBtn=e=>{console.log("TO-DO: Add product to cart.",e.id);let t=this.getProdIdFromElementId(e.id);console.log(`TO-DO: Add product with ID ${t} to cart.`),v=this.modelCart.addProductToCart(v,t,y),this.renderCartAndOrdersSummary(v,P)};// "Go to Cart" button at right panel
...
let c=this.findAncestorByClass(r,"product-item");// Qty deduct button at Cart page
if(c&&this.handleProductItem(c),r.classList.contains("cart-element__qty-mod_deduct-btn")){this.handleQtyChangeBtn(r,t,-1);return}// Qty add button at Cart page
