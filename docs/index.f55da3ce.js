...
return(""+e[2]).replace(/^((?:https?|file|ftp|(chrome|moz|safari-web)-extension):\/\/.+)\/[^/]+$/,"$1")+"/"}return"/"}(),a[e]=t),t}}),o("4G3qk").register(JSON.parse('{"ceDhw":"index.f55da3ce.js","4M8Gf":"icon-add-order.d5dcde0a.png","kSZwU":"icon-cart.dd455062.png","8HtV0":"icon-order.5ea58f4f.png","kVJvh":"admin.59777211.js"}'));var d={};d=o("260Jh").getBundleURL("ceDhw")+o("4G3qk").resolve("4M8Gf");var i={};i=o("260Jh").getBundleURL("ceDhw")+o("4G3qk").resolve("kSZwU");class c{constructor(e){this.controller=e}// FOR TEST AND TBS - REMOVE FOR PROD
...
let I={},P="",f={},_={},v=[],b=[],w={},T={},O=document.createElement("div");O.innerHTML="";let k=document.createElement("div");k.innerHTML="";let B=document.createElement("div");B.innerHTML="";// Init cross-ref Map: image - url
...
b=this.getCartItemsByUserID(P),console.log(`[*** sessionIdCartItems ***] for User ID ${P}: `,b),// 2. Set "sessionIdOrders" by sesstion's userId (sessionIdNumber):
v=this.getOrdersByUserID(P),console.log(`[*** sessionIdOrders ***] for User ID ${P}: `,v),// 3. Set "sessionIdCartDetails":
T=this.getSessionIdCartDetails(),console.log(`[*** sessionIdCartDetails ***] for User ID ${P}: `,T),// ALERT - POPOUP DISCLAMER
...
this.renderCartAndOrdersSummary(b,v),this.attachEventListenrs();// TEST-TBS ONLY:
...
resetSessionIdCartItems=()=>{b=[]};// Revert back from map an image URL by image name
...
this.updateCartPriceVariable(b,w,I.productItems,_.custHandlingFee)};// MIGHT BE A PART OF MODEL:
...
e.appendChild(this.viewOrders.createBillToAddressContentBlock(T,!0)),// Second argument ("true") is to create "Update" button
e.appendChild(this.viewOrders.createPaymentMethodContentBlock(T,!0));let t=this.viewOrders.createCheckOutCartWrapper(),r=document.createElement("div");for(let e of(r.classList="checkout-cart-items",b)){let t=this.getProductObjectById(e.itemId);// Third argument ("checkout-cart-item") is a classList to pass to 'div' element
...
this.renderCartAndOrdersSummary(b,v)};// "Add to Cart" button at detailed product card (page)
...
let t=this.getProdIdFromElementId(e.id),r=this.getDocIdFromArrayByItemId(b,t);r?this.handleQtyChangeBtn(e,1):this.addProduct(t,P),this.renderCartAndOrdersSummary(b,v)};// "Go to Cart" button at right panel or checkout panel
handleGotoCartBtn=()=>{// TEST-TBS REMOVE IN PROD
console.log("Received command to display CART."),this.updateCartPriceVariable(b,w,I.productItems,_.custHandlingFee),this.renderCartSummary(b),this.renderCartSummaryWithPrice(b,w)};// Handle Order Placement
...
let e=JSON.parse(JSON.stringify(this.modelFirebase.setNewOrderDetails(T,w)));// TEST-TBS REMOVE IN PROD
...
let t=JSON.parse(JSON.stringify(b)),r=JSON.parse(JSON.stringify(e.orderId));for(let e in // TEST-TBS REMOVE IN PROD
...
for(let e in b){console.log("[*** sessionIdCartItems[cartItem].docId ***] to delete from cartsData: ",b[e].docId);try{this.handleDeleteDocFromFirestore("cartsData",b[e].docId)}catch(e){console.error("Error deleting sessionIdCartItems from Firestore: ",e)}}// 3.B. Reset sessionIdCartItems
