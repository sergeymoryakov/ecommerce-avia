...
for(let r of(i.appendChild(n),e)){let e=this.createDataRow(r,t);i.appendChild(e)}return i};renderTable=(e,t)=>{let i=document.createElement("div");i.classList="db-section";let n=document.createElement("h2");n.classList="db-section__title",n.id=`${t}`,n.innerText=t;let r=this.createTable(e,t);r.classList="db-section__table",i.appendChild(n),i.appendChild(r),console.log(i),this.containerLeftNode.appendChild(i);// REPLACED BELOW BY PARENT ELEMENT LISTENER
// this.docAddBtnNode = document.querySelectorAll(".add-btn");
// this.docUpdateBtnNode = document.querySelectorAll(".updt-btn");
// this.docDeleteBtnNode = document.querySelectorAll(".del-btn");
};createMenuBtn=e=>{let t=document.createElement("li");t.classList="menu-btn-wrapper";let i=document.createElement("a");return i.classList="menu-btn",i.href=`#${e}`,i.innerText=`${e}`,t.appendChild(i),t};createMenu=(e,t,i,n,r)=>{// Init menu wrapper:
...
n.docId=this.adminModel.generateNewId(12),n)}handleAddBtnClick=e=>{let t=this.getCollectionFromId(e.id),i=this.getDocFromId(e.id);console.log(`clicked element in ${t}, docId: ${i}.`),console.log("elementCollection =",t);let n=this.getArrayByName(t);console.log("affectedArray =",n);let r=this.duplicateDocument(n,i);console.log("clonedObject: ",r),// Add cloned object/document to array/collection:
n.push(r),this.displayTables()};handleButtonsClick=e=>{let t=e.target;t.classList.contains("add-btn")&&this.handleAddBtnClick(t)};attachEventListeners(){this.adminView.containerLeftNode.addEventListener("click",this.handleButtonsClick),// FOR TEST- TBS ONLY - REMOVE IN PROD:
...
;try{hh.initializeAppMain()}catch(e){console.error("An error occured duting app initialization:",e),alert("Something went wrong. Please contact the app admin or try again later.")}//# sourceMappingURL=index.6c902190.js.map

//# sourceMappingURL=index.6c902190.js.map
