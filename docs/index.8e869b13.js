...
return(""+e[2]).replace(/^((?:https?|file|ftp|(chrome|moz|safari-web)-extension):\/\/.+)\/[^/]+$/,"$1")+"/"}return"/"}(),c[e]=t),t}}),i("4G3qk").register(JSON.parse('{"ceDhw":"index.8e869b13.js","4M8Gf":"icon-add-order.d5dcde0a.png","kSZwU":"icon-cart.dd455062.png","8HtV0":"icon-order.5ea58f4f.png","baUMc":"admin.ed3ff856.js"}'));var a={};a=i("260Jh").getBundleURL("ceDhw")+i("4G3qk").resolve("4M8Gf");var o={};o=i("260Jh").getBundleURL("ceDhw")+i("4G3qk").resolve("kSZwU");class d{constructor(e){this.controller=e}// FOR TEST AND TBS - REMOVE FOR PROD
...
let f={},k="",v={},y={items:8e5,handling:8e3,total:808e3},w={},b=document.createElement("div");b.innerHTML="";let I=document.createElement("div");I.innerHTML="";let P=document.createElement("div");P.innerHTML="";// Init cross-ref Map: image - url
let x=new Map,B=new class{constructor(){this.viewProducts=new d(this),this.viewCart=new s(this),this.viewOrders=new m(this),this.viewAdmin=new u,this.viewSuperadmin=new p,this.modelFirebase=new h.ModelFirebase,this.modelProducts=new C,this.modelCart=new L,this.modelOrders=new g,this.modelAdmin=new _,this.containerLeftNode=document.getElementById("containerLeft"),this.containerRightNode=document.getElementById("containerRight")}initializeAppMain=async()=>{for(let e of(// TEST AND TBS - REMOVE IN PRODUCTION
...
k=f.usersData[0].userId),// Alert - Popup inform the test name usage
...
v=this.getCartItemsByUserID(k),console.log(`Cart items (sessionIdCart) for User ID ${k}: `,v),// 2. Get orders list by user ID:
w=this.getOrdersByUserID(k),console.log(`Orders History (sessionIdOrders) for User ID ${k}: `,w),// Right Container - Render Cart Summary
this.renderCartAndOrdersSummary(v,w),// Right Container - Render Cart Title
...
handleGotoProductsBtn=()=>{this.renderProductItemsList(f.productItems),this.renderCartAndOrdersSummary(v,w)};// "Add to Cart" button at detailed product card (page)
handleAddToCartBtn=()=>{console.log("TO-DO: Add product to cart.")};// "Go to Cart" button at right panel
handleGotoCartBtn=()=>{console.log("Received command to display CART."),this.renderCartSummary(v),this.renderCartSummaryWithPrice(v,y)};// "Proceed to checkout" button at right panel
