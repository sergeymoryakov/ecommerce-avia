...
return(""+e[2]).replace(/^((?:https?|file|ftp|(chrome|moz|safari-web)-extension):\/\/.+)\/[^/]+$/,"$1")+"/"}return"/"}(),o[e]=t),t}}),c("4G3qk").register(JSON.parse('{"ceDhw":"index.469452c1.js","4M8Gf":"icon-add-order.d5dcde0a.png","kSZwU":"icon-cart.dd455062.png","8HtV0":"icon-order.5ea58f4f.png","baUMc":"admin.ed3ff856.js"}'));var i={};i=c("260Jh").getBundleURL("ceDhw")+c("4G3qk").resolve("4M8Gf");var d={};d=c("260Jh").getBundleURL("ceDhw")+c("4G3qk").resolve("kSZwU");class a{constructor(e){this.controller=e}// FOR TEST AND TBS - REMOVE FOR PROD
...
s.appendChild(u),a.appendChild(l),a.appendChild(s),o.appendChild(c),o.appendChild(i),o.appendChild(a),r.appendChild(n),r.appendChild(o),r};// Left Container - Render product items
...
let f={},k="",I={},w={},v=document.createElement("div");v.innerHTML="";let b=document.createElement("div");b.innerHTML="";let y=document.createElement("div");y.innerHTML="";// Init cross-ref Map: image - url
let M=new Map,O=new class{constructor(){this.viewProducts=new a(this),this.viewCart=new s(this),this.viewOrders=new u(this),this.viewAdmin=new m,this.viewSuperadmin=new p,this.modelFirebase=new h.ModelFirebase,this.modelProducts=new g,this.modelCart=new L,this.modelOrders=new C,this.modelAdmin=new _,this.containerLeftNode=document.getElementById("containerLeft"),this.containerRightNode=document.getElementById("containerRight")}initializeAppMain=async()=>{for(let e of(// TEST AND TBS - REMOVE IN PRODUCTION
...
M.set(e.itemImg,t)}console.log("Assigned to sessionId: ",// FOR TEST AND TBS - REMOVE IN PROD:
...
I=this.getCartItemsByUserID(k),console.log(`Cart items (sessionIdCart) for User ID ${k}: `,I),// 2. Get orders list by user ID:
...
clearContainerLeft=()=>{this.containerLeftNode.innerHTML=""};clearContainerRight=()=>{this.containerRightNode.innerHTML=""};askForImageUrlFromStorage=async e=>{let t=await this.modelFirebase.getLinkToImage(e).then(e=>e).catch(e=>{console.log("Error getting download URL: ",e)});console.log("imageURL: ",t)};getUrlByNameLocal=e=>M.get(e);getCartItemsByUserID=e=>f.cartsData.filter(t=>t.userId===e);getOrdersByUserID=e=>f.ordersData.filter(t=>t.userId===e);getImageByProductId=e=>{let t=f.productItems.filter(t=>t.id===e);// TEST-TBS - REMOVE IN PROD
// console.log("productObject: ", productObject);
...
let e=this.getProdIdFromElementId(n.id);console.log("Product ID to display: ",e);let t=this.getProductObjectById(e);this.renderProductCard(t)}};attachEventListenrs=()=>{this.containerLeftNode.addEventListener("click",this.handleButtonsClick),this.containerRightNode.addEventListener("click",this.handleButtonsClick)}};try{O.initializeAppMain()}catch(e){console.error("An error occured duting app initialization:",e),alert("Something went wrong. Please contact the app admin or try again later.")}}();//# sourceMappingURL=index.469452c1.js.map

//# sourceMappingURL=index.469452c1.js.map
