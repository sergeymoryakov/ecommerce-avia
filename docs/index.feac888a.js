...
let r={result:null};return this.Hi(e,t).next(e=>{r.result=e}).next(()=>{if(!r.result)return this.Ji(e,t,n,i).next(e=>{r.result=e})}).next(()=>{if(r.result)return;let i=new on;return this.Yi(e,t,i).next(n=>{if(r.result=n,this.Wi)return this.Zi(e,t,i,n.size)})}).next(()=>r.result)}Zi(e,t,i,n){return i.documentReadCount<this.Gi?(rc()<=eB.DEBUG&&rd("QueryEngine","SDK will not create cache indexes for query:",sK(t),"since it only creates cache indexes for collection contains","more than or equal to",this.Gi,"documents"),rq.resolve()):(rc()<=eB.DEBUG&&rd("QueryEngine","Query:",sK(t),"scans",i.documentReadCount,"local documents and returns",n,"documents as results."),i.documentReadCount>this.zi*n?(rc()<=eB.DEBUG&&rd("QueryEngine","The SDK decides to create cache indexes for query:",sK(t),"as using cache indexes may help improve performance."),this.indexManager.createTargetIndexes(e,sq(t))):rq.resolve())}/**
...
return rq.resolve(null);let i=sq(t);return this.indexManager.getIndexType(e,i).next(n=>0/* IndexType.NONE */===n?null:(null!==t.limit&&1/* IndexType.PARTIAL */===n&&(i=sq(// We cannot apply a limit for targets that are served using a partial
     ...
     */Ji(e,t,i,n){return sU(t)||n.isEqual(rR.min())?rq.resolve(null):this.ji.getDocuments(e,i).next(r=>{let s=this.Xi(t,r);return this.es(t,s,i,n)?rq.resolve(null):(rc()<=eB.DEBUG&&rd("QueryEngine","Re-using previous result from %s to execute query: %s",n.toString(),sK(t)),this.ts(e,s,t,/**
...
(e.v_.set(t.targetId,t),ox(e)?oP(e):oj(e).Yo()&&oL(e,t))}/**
 ...
 */function oR(e,t){e.N_.Le(t),oj(e).h_(t)}function oP(e){e.N_=new aD({getRemoteKeysForTarget:t=>e.remoteSyncer.getRemoteKeysForTarget(t),lt:t=>e.v_.get(t)||null,st:()=>e.datastore.serializer.databaseId}),oj(e).start(),e.O_.y_()}/**
 * Returns whether the watch stream should be started because it's necessary
 * and has not yet been started.
 */function ox(e){return oO(e)&&!oj(e).Jo()&&e.v_.size>0}function oO(e){return 0===e.F_.size}async function oM(e){e.v_.forEach((t,i)=>{oL(e,t)})}async function oF(e,t){e.N_=void 0,ox(e)?(e.O_.b_(t),oP(e)):// The online state is set to unknown because there is no active attempt
 ...
 */function oz(e,t){if(rf("AsyncQueue",`${t}: ${e}`),rj(e))return new rv(ry.UNAVAILABLE,`${t}: ${e}`);throw e}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
r=o.targetId,a.sharedClientState.addLocalQueryTarget(r),s=o.view.Va();else{let e=await (i=a.localStore,n=sq(t),i.persistence.runTransaction("Allocate target","readwrite",e=>{let t;return i.Kr.getTargetData(e,n).next(r=>r?// previous targetID.
    ...
    (t=r,rq.resolve(t)):i.Kr.allocateTargetId(e).next(r=>(t=new aK(n,r,"TargetPurposeListen"/* TargetPurpose.Listen */,e.currentSequenceNumber),i.Kr.addTargetData(e,t).next(()=>t))))}).then(e=>{// If Multi-Tab is enabled, the existing target data may be newer than
// the in-memory data
 ...
 * properties.
 */class lz{/** @hideconstructor */constructor(e,t,i,n){this._firestore=e,this._userDataWriter=t,this._snapshot=n,this.metadata=new lq(n.hasPendingWrites,n.fromCache),this.query=i}/** An array of all the documents in the `QuerySnapshot`. */get docs(){let e=[];return this.forEach(t=>e.push(t)),e}/** The number of documents in the `QuerySnapshot`. */get size(){return this._snapshot.docs.size}/** True if there are no documents in the `QuerySnapshot`. */get empty(){return 0===this.size}/**
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function(e,t,i){if(!i)throw new rv(ry.INVALID_ARGUMENT,`Function ${e}() cannot be called with an empty ${t}.`)}("collection","path",t),e instanceof lS){let n=rx.fromString(t,...i);return l_(n),new lD(e,/* converter= */null,n)}{if(!(e instanceof lA||e instanceof lD))throw new rv(ry.INVALID_ARGUMENT,"Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");let n=e._path.child(rx.fromString(t,...i));return l_(n),new lD(e.firestore,/* converter= */null,n)}}(lG,e)),i=[];// console.log(newCollection);
...
document.addEventListener("input",function(e){if("INPUT"===e.target.tagName){let t=e.target.id,i=e.target.value,n=he(t,i);console.log(n)}});const ht=new class{constructor(){this.adminView=new h,this.adminModel=new u,this.adminFirebase=new lQ}initializeAppMain=async()=>{console.log("HELLO! INIT CARRIED OUT SUCCESFULLY"),this.adminView.checkModuleLinkage(),this.adminModel.checkModuleLinkage(),this.adminFirebase.checkModuleLinkage(),l6=await this.adminFirebase.get(l2),l5=await this.adminFirebase.get(l0),l3=await this.adminFirebase.get(l4),l7=await this.adminFirebase.get(l9),l8=await this.adminFirebase.get(l1),console.log("usersData: ",lW),console.log("usersDataDb: ",l6),this.displayTables(),this.adminView.renderMenu(l0,l1,l2,l9,l4)};displayTables=async()=>{this.adminView.clearContainerLeft(),this.adminView.renderTable(lW,l2),this.adminView.renderTable(l6,"usersDataDb"),this.adminView.renderTable(lY,l0),this.adminView.renderTable(l5,"customersDataDb"),this.adminView.renderTable(lJ,l4),this.adminView.renderTable(l3,"cartsDataDb"),this.adminView.renderTable(lZ,l9),this.adminView.renderTable(l7,"ordersDataDb"),this.adminView.renderTable(lX,l1),this.adminView.renderTable(l8,"productItemsDb")}};try{ht.initializeAppMain()}catch(e){console.error("An error occured duting app initialization:",e),alert("Something went wrong. Please contact the app admin or try again later.")}//# sourceMappingURL=index.feac888a.js.map

//# sourceMappingURL=index.feac888a.js.map
