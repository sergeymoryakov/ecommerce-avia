...
s.appendChild(a),c.appendChild(h),l.appendChild(c),u.appendChild(d),u.appendChild(p),l.appendChild(u),f.appendChild(m),f.appendChild(g),l.appendChild(f),y.appendChild(v),y.appendChild(w),l.appendChild(y),_.appendChild(E),_.appendChild(C),l.appendChild(_),n&&(I.appendChild(S),I.appendChild(A),I.appendChild(D)),b.appendChild(T),b.appendChild(I),l.appendChild(b),o.appendChild(l),i.appendChild(s),i.appendChild(o),i};// Left Container - Create Cart Page
...
let s=e[r++],a=e[r++],o=e[r++],l=((7&i)<<18|(63&s)<<12|(63&a)<<6|63&o)-65536;t[n++]=String.fromCharCode(55296+(l>>10)),t[n++]=String.fromCharCode(56320+(1023&l))}else{let s=e[r++],a=e[r++];t[n++]=String.fromCharCode((15&i)<<12|(63&s)<<6|63&a)}}return t.join("")},B={/**
     ...
     */encodeByteArray(e,t){if(!Array.isArray(e))throw Error("encodeByteArray takes an array as a parameter");this.init_();let r=t?this.byteToCharMapWebSafe_:this.byteToCharMap_,n=[];for(let t=0;t<e.length;t+=3){let i=e[t],s=t+1<e.length,a=s?e[t+1]:0,o=t+2<e.length,l=o?e[t+2]:0,c=i>>2,h=(3&i)<<4|a>>4,u=(15&a)<<2|l>>6,d=63&l;o||(d=64,s||(u=64)),n.push(r[c],r[h],r[u],r[d])}return n.join("")},/**
 ...
 */const ec=[];(e1=e6||(e6={}))[e1.DEBUG=0]="DEBUG",e1[e1.VERBOSE=1]="VERBOSE",e1[e1.INFO=2]="INFO",e1[e1.WARN=3]="WARN",e1[e1.ERROR=4]="ERROR",e1[e1.SILENT=5]="SILENT";const eh={debug:e6.DEBUG,verbose:e6.VERBOSE,info:e6.INFO,warn:e6.WARN,error:e6.ERROR,silent:e6.SILENT},eu=e6.INFO,ed={[e6.DEBUG]:"log",[e6.VERBOSE]:"log",[e6.INFO]:"info",[e6.WARN]:"warn",[e6.ERROR]:"error"},ep=(e,t,...r)=>{if(t<e.logLevel)return;let n=new Date().toISOString(),i=ed[t];if(i)console[i](`[${n}]  ${e.name}:`,...r);else throw Error(`Attempted to log a message with an invalid logType (value: ${t})`)};class ef{/**
...
for(let r of(ex.set(t,e),eO.values()))!/**
 * @param component - the component being added to this app's container
 *
 * @internal
 */function(e,t){try{e.container.addComponent(t)}catch(r){eL.debug(`Component ${t.name} failed to register with FirebaseApp ${e.name}`,r)}}(r,e);return!0}/**
 *
 ...
 *
 * @internal
 */function eF(e,t){let r=e.container.getProvider("heartbeat").getImmediate({optional:!0});return r&&r.triggerHeartbeat(),e.container.getProvider(t)}const eU=new ee("app","Firebase",{"no-app":"No Firebase App '{$appName}' has been created - call initializeApp() first","bad-app-name":"Illegal App name: '{$appName}","duplicate-app":"Firebase App named '{$appName}' already exists with different options or config","app-deleted":"Firebase App named '{$appName}' already deleted","no-options":"Need to provide options, when not being deployed to hosting via source.","invalid-app-argument":"firebase.{$appName}() takes either no argument or a Firebase App instance.","invalid-log-argument":"First argument to `onLog` must be null or a function.","idb-open":"Error thrown when opening IndexedDB. Original error: {$originalErrorMessage}.","idb-get":"Error thrown when reading from IndexedDB. Original error: {$originalErrorMessage}.","idb-set":"Error thrown when writing to IndexedDB. Original error: {$originalErrorMessage}.","idb-delete":"Error thrown when deleting from IndexedDB. Original error: {$originalErrorMessage}."});/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 *
 * @public
 */const eV="10.4.0";function e$(e,t={}){let r=e;if("object"!=typeof t){let e=t;t={name:e}}let n=Object.assign({name:eR,automaticDataCollectionEnabled:!1},t),i=n.name;if("string"!=typeof i||!i)throw eU.create("bad-app-name"/* AppError.BAD_APP_NAME */,{appName:String(i)});if(r||(r=X()),!r)throw eU.create("no-options"/* AppError.NO_OPTIONS */);let s=eO.get(i);if(s){// return the existing app if options and config deep equal the ones in the existing app.
if(er(r,s.options)&&er(n,s.config))return s;throw eU.create("duplicate-app"/* AppError.DUPLICATE_APP */,{appName:i})}let a=new el(i);for(let e of ex.values())a.addComponent(e);let o=new eB(r,n,a);return eO.set(i,o),o}/**
...
function iN(){return ik.logLevel}function iL(e,...t){if(ik.logLevel<=e6.DEBUG){let r=t.map(iO);ik.debug(`Firestore (${iD}): ${e}`,...r)}}function iR(e,...t){if(ik.logLevel<=e6.ERROR){let r=t.map(iO);ik.error(`Firestore (${iD}): ${e}`,...r)}}/**
 * @internal
 */function iP(e,...t){if(ik.logLevel<=e6.WARN){let r=t.map(iO);ik.warn(`Firestore (${iD}): ${e}`,...r)}}/**
 ...
 */class i${constructor(e){this.token=e,/**
         ...
         * outstanding.
         */this.i=0,this.forceRefresh=!1,this.auth=null}start(e,t){let r=this.i,n=e=>this.i!==r?(r=this.i,t(e)):Promise.resolve(),i=new iU;this.o=()=>{this.i++,this.currentUser=this.u(),i.resolve(),i=new iU,e.enqueueRetryable(()=>n(this.currentUser))};let s=()=>{let t=i;e.enqueueRetryable(async()=>{await t.promise,await n(this.currentUser)})},a=e=>{iL("FirebaseAuthCredentialsProvider","Auth detected"),this.auth=e,this.auth.addAuthTokenListener(this.o),s()};this.t.onInit(e=>a(e)),// a chance to register itself with the component framework before we
...
e.enqueueRetryable(()=>t(iA.FIRST_PARTY))}shutdown(){}invalidateToken(){}}class iz{constructor(e){this.value=e,this.type="AppCheck",this.headers=new Map,e&&e.length>0&&this.headers.set("x-firebase-appcheck",this.value)}}class iK{constructor(e){this.A=e,this.forceRefresh=!1,this.appCheck=null,this.R=null}start(e,t){let r=e=>{null!=e.error&&iL("FirebaseAppCheckTokenProvider",`Error getting App Check token; using placeholder token instead. Error: ${e.error.message}`);let r=e.token!==this.R;return this.R=e.token,iL("FirebaseAppCheckTokenProvider",`Received ${r?"new":"existing"} token.`),r?t(e.token):Promise.resolve()};this.o=t=>{e.enqueueRetryable(()=>r(t))};let n=e=>{iL("FirebaseAppCheckTokenProvider","AppCheck detected"),this.appCheck=e,this.appCheck.addTokenListener(this.o)};this.A.onInit(e=>n(e)),// a chance to register itself with the component framework.
setTimeout(()=>{if(!this.appCheck){let e=this.A.getImmediate({optional:!0});e?n(e):iL("FirebaseAppCheckTokenProvider","AppCheck not yet detected")}},0)}getToken(){let e=this.forceRefresh;return this.forceRefresh=!1,this.appCheck?this.appCheck.getToken(e).then(e=>e?("string"==typeof e.token||ix(),this.R=e.token,new iz(e.token)):null):Promise.resolve(null)}invalidateToken(){this.forceRefresh=!0}shutdown(){this.appCheck&&this.appCheck.removeTokenListener(this.o)}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
t.push(...r.split("/").filter(e=>e.length>0))}return new iZ(t)}static emptyPath(){return new iZ([])}}const i0=/^[_a-zA-Z][_a-zA-Z0-9]*$/;/**
 ...
 */class sO{constructor(e){this.value=e}static empty(){return new sO({mapValue:{}})}/**
     ...
     */field(e){if(e.isEmpty())return this.value;{let t=this.value;for(let r=0;r<e.length-1;++r)if(!sL(t=(t.mapValue.fields||{})[e.get(r)]))return null;return(t=(t.mapValue.fields||{})[e.lastSegment()])||null}}/**
...
let e=this.getFieldsMap(t);this.applyChanges(e,r,n),r={},n=[],t=i.popLast()}e?r[i.lastSegment()]=sR(e):n.push(i.lastSegment())});let i=this.getFieldsMap(t);this.applyChanges(i,r,n)}/**
     ...
     */getFieldsMap(e){let t=this.value;t.mapValue.fields||(t.mapValue={fields:{}});for(let r=0;r<e.length;++r){let n=t.mapValue.fields[e.get(r)];sL(n)&&n.mapValue.fields||(n={mapValue:{fields:{}}},t.mapValue.fields[e.get(r)]=n),t=n}return t.mapValue.fields}/**
     * Modifies `fieldsMap` by adding, replacing or deleting the specified
     * entries.
     */applyChanges(e,t,r){for(let n of(st(t,(t,r)=>e[t]=r),r))delete e[n]}clone(){return new sO(sR(this.value))}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const as=new sn(i2.comparator),aa=new sn(i2.comparator);function ao(...e){let t=aa;for(let r of e)t=t.insert(r.key,r);return t}function al(e){let t=aa;return e.forEach((e,r)=>t=t.insert(e,r.overlayedDocument)),t}function ac(){return new ai(e=>e.toString(),(e,t)=>e.isEqual(t))}const ah=new sn(i2.comparator),au=new sa(i2.comparator);function ad(...e){let t=au;for(let r of e)t=t.add(r);return t}const ap=new sa(iQ);/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
if(null===t)return e.isNoDocument()?new aU(e.key,aA.none()):new aP(e.key,e.data,aA.none());{let r=e.data,n=sO.empty(),i=new sa(i1.comparator);for(let e of t.fields)if(!i.has(e)){let t=r.field(e);// If we are deleting a nested field, we take the immediate parent as
...
null===t&&e.length>1&&(e=e.popLast(),t=r.field(e)),null===t?n.delete(e):n.set(e,t),i=i.add(e)}return new aO(e.key,n,new sl(i.toArray()),aA.none())}}/**
 ...
 */function aL(e,t,r,n){return e instanceof aP?function(e,t,r,n){if(!aD(e.precondition,t))// caused a name collision).
 ...
 */class aV{/**
     ...
     * backend.
     */applyToRemoteDocument(e,t){let r=t.mutationResults;for(let t=0;t<this.mutations.length;t++){let i=this.mutations[t];if(i.key.isEqual(e.key)){var n;n=r[t],i instanceof aP?function(e,t,r){// Unlike setMutationApplyToLocalView, if we're applying a mutation to a
// remote document the server has accepted the mutation so the precondition
// must have held.
let n=e.value.clone(),i=aM(e.fieldTransforms,t,r.transformResults);n.setAll(i),t.convertToFoundDocument(r.version,n).setHasCommittedMutations()}(i,e,n):i instanceof aO?function(e,t,r){if(!aD(e.precondition,t))// matched on the backend. We therefore must not have the expected version
...
for(let r of this.baseMutations)r.key.isEqual(e.key)&&(t=aL(r,e,t,this.localWriteTime));// Second, apply all user-provided mutations.
for(let r of this.mutations)r.key.isEqual(e.key)&&(t=aL(r,e,t,this.localWriteTime));return t}/**
...
// to O(n).
let r=ac();return this.mutations.forEach(n=>{let i=e.get(n.key),s=i.overlayedDocument,a=this.applyToLocalView(s,i.mutatedFields);// Set mutatedFields to null if the document is only from local mutations.
// This creates a Set or Delete mutation, instead of trying to create a
// patch mutation as the overlay.
a=t.has(n.key)?null:a;let o=aN(s,a);null!==o&&r.set(n.key,o),s.isValidDocument()||s.convertToNoDocument(iY.min())}),r}keys(){return this.mutations.reduce((e,t)=>e.add(t.key),ad())}isEqual(e){return this.batchId===e.batchId&&iW(this.mutations,e.mutations,(e,t)=>aR(e,t))&&iW(this.baseMutations,e.baseMutations,(e,t)=>aR(e,t))}}/** The result of applying a mutation batch to the backend. */class a${constructor(e,t,r,/**
     ...
     */static from(e,t,r){e.mutations.length===r.length||ix();let n=ah,i=e.mutations;for(let e=0;e<i.length;e++)n=n.insert(i[e].key,r[e].version);return new a$(e,t,r,n)}}/**
 * @license
 * Copyright 2022 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 */class aJ{constructor(/** The new document applies to all of these targets. */e,/** The new document is removed from all of these targets. */t,/** The key of the document for this change. */r,/**
     ...
     */r=sh.EMPTY_BYTE_STRING,n=null){this.state=e,this.targetIds=t,this.resumeToken=r,this.cause=n}}/** Tracks the internal state of a Watch target. */class a1{constructor(){/**
...
this.Ze(t),this.We=this.We.insert(t,2/* BloomFilterApplicationStatus.FalsePositive */===i?"TargetPurposeExistenceFilterMismatchBloom"/* TargetPurpose.ExistenceFilterMismatchBloom */:"TargetPurposeExistenceFilterMismatch"/* TargetPurpose.ExistenceFilterMismatch */))}}}}/**
     ...
     * filter.
     */nt(e){let t,r;let n=e.ye.unchangedNames;if(!n||!n.bits)return null;let{bits:{bitmap:i="",padding:s=0},hashCount:a=0}=n;try{t=sf(i).toUint8Array()}catch(e){if(e instanceof sc)return iP("Decoding the base64 bloom filter in existence filter failed ("+e.message+"); ignoring the bloom filter and falling back to full re-query."),null;throw e}try{// BloomFilter throws error if the inputs are invalid.
r=new aQ(t,s,a)}catch(e){return iP(e instanceof aW?"BloomFilter error: ":"Applying bloom filter failed: ",e),null}return 0===r.Ae?null:r}/**
...
n.Be(t),this.Ue=this.Ue.insert(t,this.ct(t).delete(e)),r&&(this.$e=this.$e.insert(t,r))}removeTarget(e){this.Ke.delete(e)}/**
     ...
     * specified target.
     */ut(e,t){return this.Qe.getRemoteKeysForTarget(e).has(t)}}function a9(){return new sn(i2.comparator)}function a4(){return new sn(i2.comparator)}const a6={asc:"ASCENDING",desc:"DESCENDING"},a5={"<":"LESS_THAN","<=":"LESS_THAN_OR_EQUAL",">":"GREATER_THAN",">=":"GREATER_THAN_OR_EQUAL","==":"EQUAL","!=":"NOT_EQUAL","array-contains":"ARRAY_CONTAINS",in:"IN","not-in":"NOT_IN","array-contains-any":"ARRAY_CONTAINS_ANY"},a3={and:"AND",or:"OR"};/**
 ...
 */class of{constructor(/** The target being listened to. */e,/**
     ...
     * was modified.
     */n,/** The latest snapshot version seen for this target. */i=iY.min(),s=iY.min(),a=sh.EMPTY_BYTE_STRING,o=null){this.target=e,this.targetId=t,this.purpose=r,this.sequenceNumber=n,this.snapshotVersion=i,this.lastLimboFreeSnapshotVersion=s,this.resumeToken=a,this.expectedCount=o}/** Creates a new target data instance with an updated sequence number. */withSequenceNumber(e){return new of(this.target,this.targetId,this.purpose,e,this.snapshotVersion,this.lastLimboFreeSnapshotVersion,this.resumeToken,this.expectedCount)}/**
 ...
 */class ob{constructor(e,t,r,n){this.remoteDocumentCache=e,this.mutationQueue=t,this.documentOverlayCache=r,this.indexManager=n}/**
     ...
     * state for it.
     */getDocument(e,t){let r=null;return this.documentOverlayCache.getOverlay(e,t).next(n=>(r=n,this.remoteDocumentCache.getEntry(e,t))).next(e=>(null!==r&&aL(r.mutation,e,sl.empty(),iX.now()),e))}/**
     ...
     *   to be recalculated.
     */getLocalViewOfDocuments(e,t,r=ad()){let n=ac();return this.populateOverlays(e,n,t).next(()=>this.computeViews(e,t,n,r).next(e=>{let t=ao();return e.forEach((e,r)=>{t=t.insert(e,r.overlayedDocument)}),t}))}/**
     * Gets the overlayed documents for the given document map, which will include
     * the local view of those documents and a `FieldMask` indicating which fields
     * are mutated locally, `null` if overlay is a Set or Delete mutation.
     */getOverlayedDocuments(e,t){let r=ac();return this.populateOverlays(e,r,t).next(()=>this.computeViews(e,t,r,ad()))}/**
     ...
     */computeViews(e,t,r,n){let i=as,s=ac(),a=ac();return t.forEach((e,t)=>{let a=r.get(t.key);// Recalculate an overlay if the document's existence state changed due to
...
n.has(t.key)&&(void 0===a||a.mutation instanceof aO)?i=i.insert(t.key,t):void 0!==a?(s.set(t.key,a.mutation.getFieldMask()),aL(a.mutation,t,a.mutation.getFieldMask(),iX.now())):// Using EMPTY to indicate there is no overlay for the document.
s.set(t.key,sl.empty())}),this.recalculateAndSaveOverlays(e,i).next(e=>(e.forEach((e,t)=>s.set(e,t)),t.forEach((e,t)=>{var r;return a.set(e,new oC(t,null!==(r=s.get(e))&&void 0!==r?r:null))}),a))}recalculateAndSaveOverlays(e,t){let r=ac(),n=new sn((e,t)=>e-t),i=ad();return this.mutationQueue.getAllMutationBatchesAffectingDocumentKeys(e,t).next(e=>{for(let i of e)i.keys().forEach(e=>{let s=t.get(e);if(null===s)return;let a=r.get(e)||sl.empty();a=i.applyToLocalView(s,a),r.set(e,a);let o=(n.get(i.batchId)||ad()).add(e);n=n.insert(i.batchId,o)})}).next(()=>{let s=[],a=n.getReverseIterator();// Iterate in descending order of batch IDs, and skip documents that are
// already saved.
for(;a.hasNext();){let n=a.getNext(),o=n.key,l=n.value,c=ac();l.forEach(e=>{if(!i.has(e)){let n=aN(t.get(e),r.get(e));null!==n&&c.set(e,n),i=i.add(e)}}),s.push(this.documentOverlayCache.saveOverlays(e,o,c))}return i5.waitFor(s)}).next(()=>r)}/**
 ...
 */return i2.isDocumentKey(t.path)&&null===t.collectionGroup&&0===t.filters.length?this.getDocumentsMatchingDocumentQuery(e,t.path):null!==t.collectionGroup?this.getDocumentsMatchingCollectionGroupQuery(e,t,r,n):this.getDocumentsMatchingCollectionQuery(e,t,r,n)}/**
     ...
     */getNextDocuments(e,t,r,n){return this.remoteDocumentCache.getAllFromCollectionGroup(e,t,r,n).next(i=>{let s=n-i.size>0?this.documentOverlayCache.getOverlaysForCollectionGroup(e,t,r.largestBatchId,n-i.size):i5.resolve(ac()),a=-1,o=i;return s.next(t=>i5.forEach(t,(t,r)=>(a<r.largestBatchId&&(a=r.largestBatchId),i.get(t)?i5.resolve():this.remoteDocumentCache.getEntry(e,t).next(e=>{o=o.insert(t,e)}))).next(()=>this.populateOverlays(e,t,i)).next(()=>this.computeViews(e,o,t,ad())).next(e=>({batchId:a,changes:al(e)})))})}getDocumentsMatchingDocumentQuery(e,t){// Just do a simple document lookup.
...
let r=ao();return e.forEach((e,n)=>{let s=i.get(e);void 0!==s&&aL(s.mutation,n,sl.empty(),iX.now()),ar(t,n)&&(r=r.insert(e,n))}),r})}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 */function(e){let t=function(e){var t,r,n,i,s,a,o,l;let c,h=function(e){let t=oi(e);// In v1beta1 queries for collections at the root did not have a trailing
// "/documents". In v1 all resource paths contain "/documents". Preserve the
// ability to read the v1beta1 form for compatibility with queries persisted
// in the local target cache.
return 4===t.length?iZ.emptyPath():oc(t)}(e.parent),u=e.structuredQuery,d=u.from?u.from.length:0,p=null;if(d>0){1===d||ix();let e=u.from[0];e.allDescendants?p=e.collectionId:h=h.child(e.collectionId)}let f=[];u.where&&(f=function(e){var t;let r=function e(t){return void 0!==t.unaryFilter?function(e){switch(e.unaryFilter.op){case"IS_NAN":let t=od(e.unaryFilter.field);return s$.create(t,"=="/* Operator.EQUAL */,{doubleValue:NaN});case"IS_NULL":let r=od(e.unaryFilter.field);return s$.create(r,"=="/* Operator.EQUAL */,{nullValue:"NULL_VALUE"});case"IS_NOT_NAN":let n=od(e.unaryFilter.field);return s$.create(n,"!="/* Operator.NOT_EQUAL */,{doubleValue:NaN});case"IS_NOT_NULL":let i=od(e.unaryFilter.field);return s$.create(i,"!="/* Operator.NOT_EQUAL */,{nullValue:"NULL_VALUE"});default:return ix()}}(t):void 0!==t.fieldFilter?s$.create(od(t.fieldFilter.field),function(e){switch(e){case"EQUAL":return"=="/* Operator.EQUAL */;case"NOT_EQUAL":return"!="/* Operator.NOT_EQUAL */;case"GREATER_THAN":return">"/* Operator.GREATER_THAN */;case"GREATER_THAN_OR_EQUAL":return">="/* Operator.GREATER_THAN_OR_EQUAL */;case"LESS_THAN":return"<"/* Operator.LESS_THAN */;case"LESS_THAN_OR_EQUAL":return"<="/* Operator.LESS_THAN_OR_EQUAL */;case"ARRAY_CONTAINS":return"array-contains"/* Operator.ARRAY_CONTAINS */;case"IN":return"in"/* Operator.IN */;case"NOT_IN":return"not-in"/* Operator.NOT_IN */;case"ARRAY_CONTAINS_ANY":return"array-contains-any"/* Operator.ARRAY_CONTAINS_ANY */;default:return ix()}}(t.fieldFilter.op),t.fieldFilter.value):void 0!==t.compositeFilter?sj.create(t.compositeFilter.filters.map(t=>e(t)),function(e){switch(e){case"AND":return"and"/* CompositeOperator.AND */;case"OR":return"or"/* CompositeOperator.OR */;default:return ix()}}(t.compositeFilter.op)):ix()}(e);return r instanceof sj&&sH(t=r)&&sq(t)?r.getFilters():[r]}(u.where));let m=[];u.orderBy&&(m=u.orderBy.map(e=>new sB(od(e.field),function(e){switch(e){case"ASCENDING":return"asc"/* Direction.ASCENDING */;case"DESCENDING":return"desc"/* Direction.DESCENDING */;default:return}}(e.direction))));let g=null;u.limit&&(g=null==(c="object"==typeof(t=u.limit)?t.value:t)?null:c);let y=null;u.startAt&&(y=function(e){let t=!!e.before,r=e.values||[];return new sM(r,t)}(u.startAt));let v=null;return u.endAt&&(v=function(e){let t=!e.before,r=e.values||[];return new sM(r,t)}(u.endAt)),r=h,n=p,i=m,s=f,a=g,o=y,l=v,new s4(r,n,i,s,a,"F"/* LimitType.First */,o,l)}({parent:e.parent,structuredQuery:e.structuredQuery});return"LAST"===e.limitType?s7(t,t.limit,"L"/* LimitType.Last */):t}(t.bundledQuery),readTime:or(t.readTime)}),i5.resolve()}}/**
 * @license
 * Copyright 2022 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
this.overlays=new sn(i2.comparator),this.Pr=new Map}getOverlay(e,t){return i5.resolve(this.overlays.get(t))}getOverlays(e,t){let r=ac();return i5.forEach(t,t=>this.getOverlay(e,t).next(e=>{null!==e&&r.set(t,e)})).next(()=>r)}saveOverlays(e,t,r){return r.forEach((r,n)=>{this.It(e,t,n)}),i5.resolve()}removeOverlaysForBatchId(e,t,r){let n=this.Pr.get(r);return void 0!==n&&(n.forEach(e=>this.overlays=this.overlays.remove(e)),this.Pr.delete(r)),i5.resolve()}getOverlaysForCollection(e,t,r){let n=ac(),i=t.length+1,s=new i2(t.child("")),a=this.overlays.getIteratorFrom(s);for(;a.hasNext();){let e=a.getNext().value,s=e.getKey();if(!t.isPrefixOf(s.path))break;// Documents from sub-collections
s.path.length===i&&e.largestBatchId>r&&n.set(e.getKey(),e)}return i5.resolve(n)}getOverlaysForCollectionGroup(e,t,r,n){let i=new sn((e,t)=>e-t),s=this.overlays.getIterator();for(;s.hasNext();){let e=s.getNext().value;if(e.getKey().getCollectionGroup()===t&&e.largestBatchId>r){let t=i.get(e.largestBatchId);null===t&&(t=ac(),i=i.insert(e.largestBatchId,t)),t.set(e.getKey(),e)}}let a=ac(),o=i.getIterator();for(;o.hasNext()&&(o.getNext().value.forEach((e,t)=>a.set(e,t)),!(a.size()>=n)););return i5.resolve(a)}It(e,t,r){// Remove the association of the overlay to its batch id.
 ...
 */class oN extends oE{constructor(e){super(),this.ur=e}applyChanges(e){let t=[];return this.changes.forEach((r,n)=>{n.isValidDocument()?t.push(this.ur.addEntry(e,n)):this.ur.removeEntry(r)}),i5.waitFor(t)}getFromCache(e,t){return this.ur.getEntry(e,t)}getAllFromCache(e,t){return this.ur.getEntries(e,t)}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
return i5.resolve(null);let r=s3(t);return this.indexManager.getIndexType(e,r).next(n=>0/* IndexType.NONE */===n?null:(null!==t.limit&&1/* IndexType.PARTIAL */===n&&(r=s3(// We cannot apply a limit for targets that are served using a partial
     ...
     */Ji(e,t,r,n){return s6(t)||n.isEqual(iY.min())?i5.resolve(null):this.ji.getDocuments(e,r).next(i=>{let s=this.Xi(t,i);return this.es(t,s,r,n)?i5.resolve(null):(iN()<=e6.DEBUG&&iL("QueryEngine","Re-using previous result from %s to execute query: %s",n.toString(),at(t)),this.ts(e,s,t,/**
...
let i="F"/* LimitType.First */===e.limitType?t.last():t.first();return!!i&&(i.hasPendingWrites||i.version.compareTo(n)>0)}Yi(e,t,r){return iN()<=e6.DEBUG&&iL("QueryEngine","Using full collection scan to execute query:",at(t)),this.ji.getDocumentsMatchingQuery(e,t,i9.min(),r)}/**
...
iL("LocalStore",`Failed to update sequence numbers for target ${t}: ${e}`)}e.rs=e.rs.remove(t),e.ss.delete(n.target)}/**
...
return!1}bo(e,t,r,n,i){let s=oQ(),a=this.Do(e,t);iL("RestConnection",`Sending RPC '${e}' ${s}:`,a,r);let o={"google-cloud-resource-prefix":this.yo,"x-goog-request-params":this.wo};return this.Co(o,n,i),this.vo(e,a,o,r).then(t=>(iL("RestConnection",`Received RPC '${e}' ${s}: `,t),t),t=>{throw iP("RestConnection",`RPC '${e}' ${s} failed with error: `,t,"url: ",a,"request:",r),t})}Fo(e,t,r,n,i,s){// The REST API automatically aggregates all of the streamed results, so we
...
return m(u,iC.EventType.OPEN,()=>{p||iL(oY,`RPC '${e}' stream ${n} transport opened.`)}),m(u,iC.EventType.CLOSE,()=>{p||(p=!0,iL(oY,`RPC '${e}' stream ${n} transport closed`),f.mo())}),m(u,iC.EventType.ERROR,t=>{p||(p=!0,iP(oY,`RPC '${e}' stream ${n} transport errored:`,t),f.mo(new iF(iM.UNAVAILABLE,"The operation could not be completed")))}),m(u,iC.EventType.MESSAGE,t=>{var r;if(!p){let i=t.data[0];i||ix();// TODO(b/35143891): There is a bug in One Platform that caused errors
// (and only errors) to be wrapped in an extra array. To be forward
// compatible with the bug we need to check either condition. The latter
// can be removed once the fix has been rolled out.
// Use any because msgData.error is not typed.
let s=i.error||(null===(r=i[0])||void 0===r?void 0:r.error);if(s){iL(oY,`RPC '${e}' stream ${n} received error:`,s);// error.status will be a string like 'OK' or 'NOT_FOUND'.
let t=s.status,r=/**
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
let t=h[e];if(void 0!==t)return aH(t)}(t),i=s.message;void 0===r&&(r=iM.INTERNAL,i="Unknown error status: "+t+" with message "+s.message),p=!0,f.mo(new iF(r,i)),u.close()}else iL(oY,`RPC '${e}' stream ${n} received:`,i),f.fo(i)}}),m(a,i_.STAT_EVENT,t=>{t.stat===iE.PROXY?iL(oY,`RPC '${e}' stream ${n} detected buffering proxy`):t.stat===iE.NOPROXY&&iL(oY,`RPC '${e}' stream ${n} detected no buffering proxy`)}),setTimeout(()=>{// Technically we could/should wait for the WebChannel opened event,
...
n>0&&iL("ExponentialBackoff",`Backing off for ${n} ms (base delay: ${this.Bo} ms, delay with jitter: ${t} ms, last attempt: ${r} ms ago)`),this.Lo=this._i.enqueueAfterDelay(this.timerId,n,()=>(this.ko=Date.now(),e())),// bounds.
...
// if unset
let s="NO_CHANGE"===(n=t.targetChange.targetChangeType||"NO_CHANGE")?0/* WatchTargetChangeState.NoChange */:"ADD"===n?1/* WatchTargetChangeState.Added */:"REMOVE"===n?2/* WatchTargetChangeState.Removed */:"CURRENT"===n?3/* WatchTargetChangeState.Current */:"RESET"===n?4/* WatchTargetChangeState.Reset */:ix(),a=t.targetChange.targetIds||[],o=(i=t.targetChange.resumeToken,e.useProto3Json?(void 0===i||"string"==typeof i||ix(),sh.fromBase64String(i||"")):(void 0===i||i instanceof Uint8Array||ix(),sh.fromUint8Array(i||new Uint8Array))),l=t.targetChange.cause,c=l&&function(e){let t=void 0===e.code?iM.UNKNOWN:aH(e.code);return new iF(t,e.message||"")}(l);r=new a0(s,a,o,c||null)}else if("documentChange"in t){t.documentChange;let n=t.documentChange;n.document,n.document.name,n.document.updateTime;let i=oa(e,n.document.name),s=or(n.document.updateTime),a=n.document.createTime?or(n.document.createTime):iY.min(),o=new sO({mapValue:{fields:n.document.fields}}),l=sx.newFoundDocument(i,s,a,o),c=n.targetIds||[],h=n.removedTargetIds||[];r=new aJ(c,h,l.key,l)}else if("documentDelete"in t){t.documentDelete;let n=t.documentDelete;n.document;let i=oa(e,n.document),s=n.readTime?or(n.readTime):iY.min(),a=sx.newNoDocument(i,s),o=n.removedTargetIds||[];r=new aJ([],o,a.key,a)}else if("documentRemove"in t){t.documentRemove;let n=t.documentRemove;n.document;let i=oa(e,n.document),s=n.removedTargetIds||[];r=new aJ([],s,i,null)}else{if(!("filter"in t))return ix();{t.filter;let e=t.filter;e.targetId;let{count:n=0,unchangedNames:i}=e,s=new aq(n,i),a=e.targetId;r=new aZ(a,s)}}return r}(this.serializer,e),r=function(e){// We have only reached a consistent snapshot for the entire stream if there
...
e.O_.set("Unknown"/* OnlineState.Unknown */)}async function lc(e,t,r){if(e.O_.set("Online"/* OnlineState.Online */),t instanceof a0&&2/* WatchTargetChangeState.Removed */===t.state&&t.cause)// to raise events
try{await /** Handles an error on a target */async function(e,t){let r=t.cause;for(let n of t.targetIds)e.v_.has(n)&&(await e.remoteSyncer.rejectListen(n,r),e.v_.delete(n),e.N_.removeTarget(n))}(e,t)}catch(r){iL("RemoteStore","Failed to remove targets %s: %s ",t.targetIds.join(","),r),await lh(e,r)}else if(t instanceof aJ?e.N_.Ge(t):t instanceof aZ?e.N_.Xe(t):e.N_.He(t),!r.isEqual(iY.min()))try{let t=await oV(e.localStore);r.compareTo(t)>=0&&// We have received a target change with a global snapshot if the snapshot
...
// state.
e.v_.set(t,n.withResumeToken(sh.EMPTY_BYTE_STRING,n.snapshotVersion)),// deliberately don't send a resume token so that we get a full update.
ln(e,t);// Mark the target we send as being on behalf of an existence filter
// mismatch, but don't actually retain that in listenTargets. This ensures
// that we flag the first re-listen this way without impacting future
// listens of this target (that might happen e.g. on reconnect).
let i=new of(n.target,t,r,n.sequenceNumber);lr(e,i)}),e.remoteSyncer.applyRemoteEvent(r)}(e,r)}catch(t){iL("RemoteStore","Failed to raise snapshot:",t),await lh(e,t)}}/**
 ...
 * any retry attempt.
 */async function lh(e,t,r){if(!i3(t))throw t;e.F_.add(1/* OfflineCause.IndexedDbFailed */),await o8(e),e.O_.set("Offline"/* OnlineState.Offline */),r||// Use a simple read operation to determine if IndexedDB recovered.
// Ideally, we would expose a health check directly on SimpleDb, but
// RemoteStore only has access to persistence through LocalStore.
(r=()=>oV(e.localStore)),e.asyncQueue.enqueueRetryable(async()=>{iL("RemoteStore","Retrying IndexedDB access"),await r(),e.F_.delete(1/* OfflineCause.IndexedDbFailed */),await o7(e)})}/**
 * Executes `op`. If `op` fails, takes the network offline until `op`
 * succeeds. Returns after the first attempt.
 */function lu(e,t){return t().catch(r=>lh(e,r,t))}async function ld(e){let t=lC(e),r=e.C_.length>0?e.C_[e.C_.length-1].batchId:-1;for(;la(e)&&e.C_.length<10;)try{let n=await /**
 ...
 */function(e,t){e.C_.push(t);let r=lC(e);r.Yo()&&r.I_&&r.d_(t.mutations)}(e,n)}catch(t){await lh(e,t)}lp(e)&&lf(e)}function lp(e){return la(e)&&!lC(e).Jo()&&e.C_.length>0}function lf(e){lC(e).start()}async function lm(e){lC(e).A_()}async function lg(e){let t=lC(e);// Send the write pipeline now that the stream is established.
...
lp(e)&&lf(e)}async function lw(e,t){e.asyncQueue.verifyOperationInProgress(),iL("RemoteStore","RemoteStore received new credentials");let r=la(e);// Tear down and re-create our network streams. This will ensure we get a
...
(e.B_=(t=e.datastore,r=e.asyncQueue,n={Io:lo.bind(null,e),Eo:ll.bind(null,e),c_:lc.bind(null,e)},t.V_(),new o9(r,t.connection,t.authCredentials,t.appCheckCredentials,t.serializer,n)),e.M_.push(async t=>{t?(e.B_.Xo(),ls(e)?li(e):e.O_.set("Unknown"/* OnlineState.Unknown */)):(await e.B_.stop(),e.N_=void 0)})),e.B_}/**
 ...
 */class lD{constructor(){this.Q_=void 0,this.listeners=[]}}class lk{constructor(){this.queries=new ai(e=>ae(e),s8),this.onlineState="Unknown"/* OnlineState.Unknown */,this.K_=new Set}}async function lN(e,t){let r=t.query,n=!1,i=e.queries.get(r);if(i||(n=!0,i=new lD),n)try{i.Q_=await e.onListen(r)}catch(r){let e=lT(r,`Initialization of query '${at(t.query)}' failed`);return void t.onError(e)}e.queries.set(r,i),i.listeners.push(t),t.U_(e.onlineState),i.Q_&&t.W_(i.Q_)&&lO(e)}async function lL(e,t){let r=t.query,n=!1,i=e.queries.get(r);if(i){let e=i.listeners.indexOf(t);e>=0&&(i.listeners.splice(e,1),n=0===i.listeners.length)}if(n)return e.queries.delete(r),e.onUnlisten(r)}function lR(e,t){let r=!1;for(let n of t){let t=n.query,i=e.queries.get(t);if(i){for(let e of i.listeners)e.W_(n)&&(r=!0);i.Q_=n}}r&&lO(e)}function lP(e,t,r){let n=e.queries.get(t);if(n)for(let e of n.listeners)e.onError(r);// Remove all listeners. NOTE: We don't need to call syncEngine.unlisten()
// after an error.
...
if(e.inorderTraversal((e,t)=>{let c=n.get(e),h=ar(this.query,t)?t:null,u=!!c&&this.mutatedKeys.has(c.key),d=!!h&&(h.hasLocalMutations||// We only consider committed mutations for documents that were
// mutated during the lifetime of the view.
this.mutatedKeys.has(h.key)&&h.hasCommittedMutations),p=!1;c&&h?c.data.isEqual(h.data)?u!==d&&(r.track({type:3/* ChangeType.Metadata */,doc:h}),p=!0):this.Ia(c,h)||(r.track({type:2/* ChangeType.Modified */,doc:h}),p=!0,(o&&this.ua(h,o)>0||l&&0>this.ua(h,l))&&// This doc moved from inside the limit to outside the limit.
// That means there may be some other doc in the local cache
// that should be included instead.
(a=!0)):!c&&h?(r.track({type:0/* ChangeType.Added */,doc:h}),p=!0):c&&!h&&(r.track({type:1/* ChangeType.Removed */,doc:c}),p=!0,(o||l)&&// A doc was removed from a full limit query. We'll need to
// requery from the local cache to see if we know about some other
// doc that should be in the results.
(a=!0)),p&&(h?(s=s.add(h),i=d?i.add(e):i.delete(e)):(s=s.delete(e),i=i.delete(e)))}),null!==this.query.limit)for(;s.size>this.query.limit;){let e="F"/* LimitType.First */===this.query.limitType?s.last():s.first();s=s.delete(e.key),i=i.delete(e.key),r.track({type:1/* ChangeType.Removed */,doc:e})}return{ca:s,Pa:r,es:a,mutatedKeys:i}}Ia(e,t){// We suppress the initial change event for documents that were modified as
 ...
 */async function lj(e,t){var r,n;let i,s;let a=(e.remoteStore.remoteSyncer.applyRemoteEvent=lK.bind(null,e),e.remoteStore.remoteSyncer.getRemoteKeysForTarget=l6.bind(null,e),e.remoteStore.remoteSyncer.rejectListen=lQ.bind(null,e),e.fa.c_=lR.bind(null,e.eventManager),e.fa.xa=lP.bind(null,e.eventManager),e),o=a.ga.get(t);if(o)// already exists when EventManager calls us for the first time. This
 ...
 */function(e,t){let r=null;for(let n of e.fieldTransforms){let e=t.data.field(n.field),i=ay(n.transform,e||null);null!=i&&(null===r&&(r=sO.empty()),r.set(n.field,i))}return r||null}(e,r.get(e.key).overlayedDocument);null!=t&&// NOTE: The base state should only be applied if there's some
// existing document to override, so use a Precondition of
// exists=true
s.push(new aO(e.key,t,/**
 * Returns a FieldMask built from all fields in a MapValue.
 */function e(t){let r=[];return st(t.fields,(t,n)=>{let i=new i1([t]);if(sL(n)){let t=e(n.mapValue).fields;if(0===t.length)r.push(i);else // leaf nodes.
...
n=e.rs;let c=[];t.targetChanges.forEach((s,a)=>{var o;let l=n.get(a);if(!l)return;// Only update the remote keys if the target is still active. This
...
// limbo.
if(t.documentUpdates.forEach(r=>{t.resolvedLimboDocuments.has(r)&&c.push(e.persistence.referenceDelegate.updateLimboDocument(i,r))}),// the remote documents in advance in a single call.
c.push((s=t.documentUpdates,a=ad(),o=ad(),s.forEach(e=>a=a.add(e)),l.getEntries(i,a).next(e=>{let t=as;return s.forEach((r,n)=>{let i=e.get(r);// Check if see if there is a existence state change for this document.
...
// SharedClientState.
if(e.isPrimaryClient&&0/* OnlineStateSource.RemoteStore */===r||!e.isPrimaryClient&&1/* OnlineStateSource.SharedClientState */===r){let r;let i=[];e.ga.forEach((e,r)=>{let n=r.view.U_(t);n.snapshot&&i.push(n.snapshot)}),(n=e.eventManager).onlineState=t,r=!1,n.queries.forEach((e,n)=>{for(let e of n.listeners)e.U_(t)&&(r=!0)}),r&&lO(n),i.length&&e.fa.c_(i),e.onlineState=t,e.isPrimaryClient&&e.sharedClientState.setOnlineState(t)}}/**
 ...
 */class l8{constructor(e,t,/**
     ...
     */r,n){this.authCredentials=e,this.appCheckCredentials=t,this.asyncQueue=r,this.databaseInfo=n,this.user=iA.UNAUTHENTICATED,this.clientId=iG.V(),this.authCredentialListener=()=>Promise.resolve(),this.appCheckCredentialListener=()=>Promise.resolve(),this.authCredentials.start(r,async e=>{iL("FirestoreClient","Received user=",e.uid),await this.authCredentialListener(e),this.user=e}),this.appCheckCredentials.start(r,e=>(iL("FirestoreClient","Received new app check token=",e),this.appCheckCredentialListener(e,this.user)))}async getConfiguration(){return{asyncQueue:this.asyncQueue,databaseInfo:this.databaseInfo,clientId:this.clientId,authCredentials:this.authCredentials,appCheckCredentials:this.appCheckCredentials,initialUser:this.user,maxConcurrentLimboResolutions:100}}setCredentialChangeListener(e){this.authCredentialListener=e}setAppCheckTokenChangeListener(e){this.appCheckCredentialListener=e}/**
...
// tokens.
this.authCredentials.shutdown(),this.appCheckCredentials.shutdown(),e.resolve()}catch(r){let t=lT(r,"Failed to shutdown persistence");e.reject(t)}}),e.promise}}async function ce(e,t){e.asyncQueue.verifyOperationInProgress(),iL("FirestoreClient","Initializing OfflineComponentProvider");let r=await e.getConfiguration();await t.initialize(r);let n=r.initialUser;e.setCredentialChangeListener(async e=>{n.isEqual(e)||(await oB(t.localStore,e),n=e)}),// need to be terminated to allow the delete to succeed.
t.persistence.setDatabaseDeletedListener(()=>e.terminate()),e._offlineComponents=t}async function ct(e,t){e.asyncQueue.verifyOperationInProgress();let r=await cr(e);iL("FirestoreClient","Initializing OnlineComponentProvider");let n=await e.getConfiguration();await t.initialize(r,n),// precedence over the offline component provider.
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function co(e,t,r){if(!r)throw new iF(iM.INVALID_ARGUMENT,`Function ${e}() cannot be called with an empty ${t}.`)}/**
 ...
 *//** Returns a string describing the type / value of the provided input. */function ch(e){if(void 0===e)return"undefined";if(null===e)return"null";if("string"==typeof e)return e.length>20&&(e=`${e.substring(0,20)}...`),JSON.stringify(e);if("number"==typeof e||"boolean"==typeof e)return""+e;if("object"==typeof e){if(e instanceof Array)return"an array";{var t;let r=(t=e).constructor?t.constructor.name:null;return r?`a custom ${r} object`:"an object"}}return"function"==typeof e?"a function":ix()}function cu(e,t){if("_delegate"in e&&// Unwrap Compat types
// eslint-disable-next-line @typescript-eslint/no-explicit-any
(e=e._delegate),!(e instanceof t)){if(t.name===e.constructor.name)throw new iF(iM.INVALID_ARGUMENT,"Type does not match the expected instance. Did you pass a reference from a different Firestore SDK?");{let r=ch(e);throw new iF(iM.INVALID_ARGUMENT,`Expected type '${t.name}', but it was: ${r}`)}}return e}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
/** @hideconstructor protected */constructor(e,/**
     ...
     */t,r){this.converter=t,this._query=r,/** The type of this Firestore reference. */this.type="query",this.firestore=e}withConverter(e){return new cf(this.firestore,e,this._query)}}/**
 ...
 */class cg extends cf{/** @hideconstructor */constructor(e,t,r){super(e,t,new s4(r)),this._path=r,/** The type of this Firestore reference. */this.type="collection"}/** The collection's identifier. */get id(){return this._query.path.lastSegment()}/**
     * A string representing the path of the referenced collection (relative
     * to the root of the database).
     */get path(){return this._query.path.canonicalString()}/**
     * A reference to the containing `DocumentReference` if this is a
     * subcollection. If this isn't a subcollection, the reference is null.
     */get parent(){let e=this._path.popLast();return e.isEmpty()?null:new cm(this.firestore,/* converter= */null,new i2(e))}withConverter(e){return new cg(this.firestore,e,this._path)}}function cy(e,t,...r){if(e=ei(e),// 'undefined' and 'null'.
1==arguments.length&&(t=iG.V()),co("doc","path",t),e instanceof cp){let n=iZ.fromString(t,...r);return cl(n),new cm(e,/* converter= */null,new i2(n))}{if(!(e instanceof cm||e instanceof cg))throw new iF(iM.INVALID_ARGUMENT,"Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");let n=e._path.child(iZ.fromString(t,...r));return cl(n),new cm(e.firestore,e instanceof cg?e.converter:null,new i2(n))}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 */class cw extends cp{/** @hideconstructor */constructor(e,t,r,n){super(e,t,r,n),/**
         * Whether it's a {@link Firestore} or Firestore Lite instance.
         */this.type="firestore",this._queue=new cv,this._persistenceKey=(null==n?void 0:n.name)||"[DEFAULT]"}_terminate(){return this._firestoreClient||// The client must be initialized to ensure that all subsequent API
// usage throws an exception.
cE(this),this._firestoreClient.terminate()}}/**
 * @internal
 */function c_(e){return e._firestoreClient||cE(e),e._firestoreClient.verifyNotTerminated(),e._firestoreClient}function cE(e){var t,r,n,i,s,a;let o=e._freezeSettings(),l=(i=e._databaseId,s=(null===(t=e._app)||void 0===t?void 0:t.options.appId)||"",a=e._persistenceKey,new sv(i,s,a,o.host,o.ssl,o.experimentalForceLongPolling,o.experimentalAutoDetectLongPolling,cs(o.experimentalLongPollingOptions),o.useFetchStreams));e._firestoreClient=new l8(e._authCredentials,e._appCheckCredentials,e._queue,l),(null===(r=o.localCache)||void 0===r?void 0:r._offlineComponentProvider)&&(null===(n=o.localCache)||void 0===n?void 0:n._onlineComponentProvider)&&(e._firestoreClient._uninitializedComponentsProvider={_offlineKind:o.localCache.kind,_offline:o.localCache._offlineComponentProvider,_online:o.localCache._onlineComponentProvider})}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * classes.
 */class cL{constructor(e,t,r){this.databaseId=e,this.ignoreUndefinedProperties=t,this.serializer=r||o0(e)}/** Creates a new top-level parse context. */wu(e,t,r,n=!1){return new cN({Tu:e,methodName:t,yu:r,path:i1.emptyPath(),Ru:!1,pu:n},this.databaseId,this.serializer,this.ignoreUndefinedProperties)}}function cR(e){let t=e._freezeSettings(),r=o0(e._databaseId);return new cL(e._databaseId,!!t.ignoreUndefinedProperties,r)}class cP extends cT{_toFieldTransform(e){if(2/* UserDataSource.MergeSet */!==e.Tu)throw 1/* UserDataSource.Update */===e.Tu?e.gu(`${this._methodName}() can only appear at the top level of your update data`):e.gu(`${this._methodName}() cannot be used with set() unless you pass {merge:true}`);// No transform to add for a delete, but we need to add it to our
...
if(t.settings.Ru&&4/* UserDataSource.ArrayArgument */!==t.Tu)throw t.gu("Nested arrays are not supported");return function(e,t){let r=[],n=0;for(let i of e){let e=cO(i,t.fu(n));null==e&&// Just include nulls in the array for fields being replaced with a
// sentinel.
...
let r=new iX(e.seconds,1e3*Math.floor(e.nanoseconds/1e3));return{timestampValue:oe(t.serializer,r)}}if(e instanceof cI)return{geoPointValue:{latitude:e.latitude,longitude:e.longitude}};if(e instanceof cC)return{bytesValue:ot(t.serializer,e._byteString)};if(e instanceof cm){let r=t.databaseId,n=e.firestore._databaseId;if(!n.isEqual(r))throw t.gu(`Document reference is for database ${n.projectId}/${n.database} but should be for database ${r.projectId}/${r.database}`);return{referenceValue:on(e.firestore._databaseId||t.databaseId,e._key.path)}}throw t.gu(`Unsupported field value: ${ch(e)}`)}(e,t)}function cx(e,t){let r={};return sr(e)?// mask to ensure that the server creates a map entry.
t.path&&t.path.length>0&&t.fieldMask.push(t.path):st(e,(e,n)=>{let i=cO(n,t.Au(e));null!=i&&(r[e]=i)}),{mapValue:{fields:r}}}function cM(e){return!("object"!=typeof e||null===e||e instanceof Array||e instanceof Date||e instanceof iX||e instanceof cI||e instanceof cC||e instanceof cm||e instanceof cT)}function cF(e,t,r){if(!cM(r)||!("object"==typeof r&&null!==r&&(Object.getPrototypeOf(r)===Object.prototype||null===Object.getPrototypeOf(r)))){let n=ch(r);throw"an object"===n?t.gu(e+" a custom object"):t.gu(e+" "+n)}}/**
 * Helper that calls fromDotSeparatedString() but wraps any error thrown.
 */function cU(e,t,r){if(// FieldPath.
(t=ei(t))instanceof cb)return t._internalPath;if("string"==typeof t)return cV(e,t);throw c$("Field path arguments must be of type string or ",e,/* hasConverter= */!1,/* path= */void 0,r)}/**
 ...
 * evaluated.
 */function cV(e,t,r){if(t.search(cB)>=0)throw c$(`Invalid field path (${t}). Paths must not contain '~', '*', '/', '[', or ']'`,e,/* hasConverter= */!1,/* path= */void 0,r);try{return new cb(...t.split("."))._internalPath}catch(n){throw c$(`Invalid field path (${t}). Paths must not be empty, begin with '.', end with '.', or contain '..'`,e,/* hasConverter= */!1,/* path= */void 0,r)}}function c$(e,t,r,n,i){let s=n&&!n.isEmpty(),a=void 0!==i,o=`Function ${t}() called with invalid data`;r&&(o+=" (via `toFirestore()`)"),o+=". ";let l="";return(s||a)&&(l+=" (found",s&&(l+=` in field ${n}`),a&&(l+=` in document ${i}`),l+=")"),new iF(iM.INVALID_ARGUMENT,o+e+l)}/** Checks `haystack` if FieldPath `needle` is present. Runs in O(n). */function cj(e,t){return e.some(e=>e.isEqual(t))}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * access will return 'undefined'. You can use the `exists()` method to
 * explicitly verify a document's existence.
 */class cQ extends cq{/** @hideconstructor protected */constructor(e,t,r,n,i,s){super(e,t,r,n,s),this._firestore=e,this._firestoreImpl=e,this.metadata=i}/**
...
let t=new cW(this._firestore,this._userDataWriter,this._key,this._document,this.metadata,/* converter= */null);return this._converter.fromFirestore(t,e)}return this._userDataWriter.convertValue(this._document.data.value,e.serverTimestamps)}}/**
     ...
     */// We are using `any` here to avoid an explicit cast by our users.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
get(e,t={}){if(this._document){let r=this._document.data.field(cz("DocumentSnapshot.get",e));if(null!==r)return this._userDataWriter.convertValue(r,t.serverTimestamps)}}}/**
 * A `QueryDocumentSnapshot` contains data read from a document in your
 * Firestore database as part of a query. The document is guaranteed to exist
 * and its data can be extracted with `.data()` or `.get(<field>)` to get a
 * specific field.
 ...
 * properties.
 */class cX{/** @hideconstructor */constructor(e,t,r,n){this._firestore=e,this._userDataWriter=t,this._snapshot=n,this.metadata=new cG(n.hasPendingWrites,n.fromCache),this.query=r}/** An array of all the documents in the `QuerySnapshot`. */get docs(){let e=[];return this.forEach(t=>e.push(t)),e}/** The number of documents in the `QuerySnapshot`. */get size(){return this._snapshot.docs.size}/** True if there are no documents in the `QuerySnapshot`. */get empty(){return 0===this.size}/**
     * Enumerates all of the documents in the `QuerySnapshot`.
     *
     * @param callback - A callback to be called with a `QueryDocumentSnapshot` for
     * each document in the snapshot.
     * @param thisArg - The `this` binding for the callback.
     */forEach(e,t){this._snapshot.docs.forEach(r=>{e.call(t,new cW(this._firestore,this._userDataWriter,r.key,r,new cG(this._snapshot.mutatedKeys.has(r.key),this._snapshot.fromCache),this.query.converter))})}/**
 ...
 */class c1 extends Z{/**
     ...
     */constructor(e,t,r=0){super(c2(e),`Firebase Storage: ${t} (${c2(e)})`),this.status_=r,/**
 ...
 *
 * @internal
 */class c3{constructor(e,t){this.bucket=e,this.path_=t}get path(){return this.path_}get isRoot(){return 0===this.path.length}fullServerUrl(){let e=encodeURIComponent;return"/b/"+e(this.bucket)+"/o/"+e(this.path)}bucketOnlyServerUrl(){let e=encodeURIComponent;return"/b/"+e(this.bucket)+"/o"}static makeFromBucketSpec(e,t){let r;try{r=c3.makeFromUrl(e,t)}catch(t){// Not valid URL, use as-is. This lets you put bare bucket names in
// config.
return new c3(e,"")}if(""===r.path)return r;throw new c1(d.INVALID_DEFAULT_BUCKET,"Invalid default bucket '"+e+"'.")}static makeFromUrl(e,t){let r=null,n="([A-Za-z0-9.\\-_]+)",i=RegExp("^gs://"+n+"(/(.*))?$","i");function s(e){e.path_=decodeURIComponent(e.path)}let a=t.replace(/[.]/g,"\\."),o=RegExp(`^https?://${a}/v[A-Za-z0-9_]+/b/${n}/o(/([^?#]*).*)?$`,"i"),l=RegExp(`^https?://${t===cZ?"(?:storage.googleapis.com|storage.cloud.google.com)":t}/${n}/([^?#]*)`,"i"),c=[{regex:i,indices:{bucket:1,path:3},postModify:function(e){"/"===e.path.charAt(e.path.length-1)&&(e.path_=e.path_.slice(0,-1))}},{regex:o,indices:{bucket:1,path:3},postModify:s},{regex:l,indices:{bucket:1,path:2},postModify:s}];for(let t=0;t<c.length;t++){let n=c[t],i=n.regex.exec(e);if(i){let e=i[n.indices.bucket],t=i[n.indices.path];t||(t=""),r=new c3(e,t),n.postModify(r);break}}if(null==r)throw new c1(d.INVALID_URL,"Invalid URL '"+e+"'.");return r}}/**
 * A request whose promise always fails.
 */class c7{constructor(e){this.promise_=Promise.reject(e)}/** @inheritDoc */getPromise(){return this.promise_}/** @inheritDoc */cancel(e=!1){}}function c8(e){return"string"==typeof e||e instanceof String}function he(e,t,r,n){if(n<t)throw c4(`Invalid value for '${e}'. Expected ${t} or greater.`);if(n>r)throw c4(`Invalid value for '${e}'. Expected ${r} or less.`)}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function ht(e,t,r){let n=t;return null==r&&(n=`https://${t}`),`${r}://${n}/v0${e}`}function hr(e){let t=encodeURIComponent,r="?";for(let n in e)if(e.hasOwnProperty(n)){let i=t(n)+"="+t(e[n]);r=r+i+"&"}return r.slice(0,-1)}(a=p||(p={}))[a.NO_ERROR=0]="NO_ERROR",a[a.NETWORK_ERROR=1]="NETWORK_ERROR",a[a.ABORT=2]="ABORT";/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 */class hn{constructor(e,t,r,n,i,s,a,o,l,c,h,u=!0){this.url_=e,this.method_=t,this.headers_=r,this.body_=n,this.successCodes_=i,this.additionalRetryCodes_=s,this.callback_=a,this.errorCallback_=o,this.timeout_=l,this.progressCallback_=c,this.connectionFactory_=h,this.retry=u,this.pendingConnection_=null,this.backoffId_=null,this.canceled_=!1,this.appDelete_=!1,this.promise_=new Promise((e,t)=>{this.resolve_=e,this.reject_=t,this.start_()})}/**
     * Actually starts the retry loop.
     */start_(){/**
         * @param requestWentThrough - True if the request eventually went
         *     through, false if it hit the retry limit or was canceled.
         */let e=(e,t)=>{let r=this.resolve_,n=this.reject_,i=t.connection;if(t.wasSuccessCode)try{let e=this.callback_(i,i.getResponse());void 0!==e?r(e):r()}catch(e){n(e)}else if(null!==i){let e=c9();e.serverResponse=i.getErrorText(),n(this.errorCallback_?this.errorCallback_(i,e):e)}else if(t.canceled){let e=this.appDelete_?c6():new c1(d.CANCELED,"User canceled the upload/download.");n(e)}else{let e=new c1(d.RETRY_LIMIT_EXCEEDED,"Max retry time for operation exceeded, please try again.");n(e)}};this.canceled_?e(!1,new hi(!1,null,!0)):this.backoffId_=/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
let n=1,i=null,s=null,a=!1,o=0,l=!1;function c(...e){l||(l=!0,t.apply(null,e))}function h(t){i=setTimeout(()=>{i=null,e(d,2===o)},t)}function u(){s&&clearTimeout(s)}function d(e,...t){let r;if(l){u();return}if(e){u(),c.call(null,e,...t);return}let i=2===o||a;if(i){u(),c.call(null,e,...t);return}n<64&&/* TODO(andysoto): don't back off so quickly if we know we're offline. */(n*=2),1===o?(o=2,r=0):r=(n+Math.random())*1e3,h(r)}let p=!1;function f(e){!p&&(p=!0,u(),l||(null!==i?(e||(o=2),clearTimeout(i),h(0)):e||(o=1)))}return h(0),s=setTimeout(()=>{a=!0,f(!0)},r),f}((e,t)=>{if(t){e(!1,new hi(!1,null,!0));return}let r=this.connectionFactory_();this.pendingConnection_=r;let n=e=>{let t=e.loaded,r=e.lengthComputable?e.total:-1;null!==this.progressCallback_&&this.progressCallback_(t,r)};null!==this.progressCallback_&&r.addUploadProgressListener(n),// connection.send() never rejects, so we don't need to have a error handler or use catch on the returned promise.
...
429].indexOf(e),i=-1!==t.indexOf(e);return r||n||i}(i,this.additionalRetryCodes_)&&this.retry){let t=r.getErrorCode()===p.ABORT;e(!1,new hi(!1,null,t));return}let s=-1!==this.successCodes_.indexOf(i);e(!0,new hi(s,r))})},e,this.timeout_)}/** @inheritDoc */getPromise(){return this.promise_}/** @inheritDoc */cancel(e){this.canceled_=!0,this.appDelete_=e||!1,null!==this.backoffId_&&(0,this.backoffId_)(!1),null!==this.pendingConnection_&&this.pendingConnection_.abort()}}/**
 * A collection of information about the result of a network request.
 * @param opt_canceled - Defaults to false.
 */class hi{constructor(e,t,r){this.wasSuccessCode=e,this.connection=t,this.canceled=!!r}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 */function hs(e){var t;let r;try{r=JSON.parse(e)}catch(e){return null}return"object"!=typeof(t=r)||Array.isArray(t)?null:r}/**
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function ho(e,t){return t}class hl{constructor(e,t,r,n){this.server=e,this.local=t||e,this.writable=!!r,this.xform=n||ho}}let hc=null;/**
 ...
 */function(e){if(0===e.length)return null;let t=e.lastIndexOf("/");if(-1===t)return"";let r=e.slice(0,t);return r}(this._location.path);if(null===e)return null;let t=new c3(this._location.bucket,e);return new hf(this._service,t)}/**
     * Utility function to throw an error in methods that do not accept a root reference.
     */_throwIfRoot(e){if(""===this._location.path)throw new c1(d.INVALID_ROOT_OPERATION,"The operation '"+e+"' cannot be performed on a root reference, create a non-root reference using child, such as .child('file.png').")}}function hm(e,t){let r=null==t?void 0:t[c0];return null==r?null:c3.makeFromBucketSpec(r,e)}/**
 ...
 */class hg{constructor(/**
     ...
     */n,i){this.app=e,this._authProvider=t,this._appCheckProvider=r,this._url=n,this._firebaseVersion=i,this._bucket=null,/**
         ...
         * - host
         * - host:port
         */this._host=cZ,this._protocol="https",this._appId=null,this._deleted=!1,this._maxOperationRetryTime=12e4,this._maxUploadRetryTime=6e5,this._requests=new Set,null!=n?this._bucket=c3.makeFromBucketSpec(n,this._host):this._bucket=hm(this._host,this.app.options)}/**
...
return t.token}return null}/**
     ...
     */_makeRequest(e,t,r,n,i=!0){if(this._deleted)return new c7(c6());{let s=function(e,t,r,n,i,s,a=!0){let o=hr(e.urlParams),l=e.url+o,c=Object.assign({},e.headers);return t&&(c["X-Firebase-GMPID"]=t),null!==r&&r.length>0&&(c.Authorization="Firebase "+r),c["X-Firebase-Storage-Version"]="webjs/"+(null!=s?s:"AppManager"),null!==n&&(c["X-Firebase-AppCheck"]=n),new hn(l,e.method,c,e.body,e.successCodes,e.additionalRetryCodes,e.handler,e.errorHandler,e.timeout,e.progressCallback,i,a)}(e,this._appId,r,n,t,this._firebaseVersion,i);return this._requests.add(s),// Request removes itself from set when complete.
s.getPromise().then(()=>this._requests.delete(s),()=>this._requests.delete(s)),s}}async makeRequestWithTokens(e,t){let[r,n]=await Promise.all([this._getAuthToken(),this._getAppCheckToken()]);return this._makeRequest(e,t,r,n).getPromise()}}const hy="@firebase/storage",hv="0.11.2",hw="storage";function h_(e,t){return function(e,t){if(!(t&&/^[A-Za-z]+:\/\//.test(t)))return(/**
 * Returns a firebaseStorage.Reference for the given path in the default
 * bucket.
 */function e(t,r){if(t instanceof hg){if(null==t._bucket)throw new c1(d.NO_DEFAULT_BUCKET,"No default bucket found. Did you set the '"+c0+"' property when initializing the app?");let n=new hf(t,t._bucket);return null!=r?e(n,r):n}return(// ref is a Reference
void 0!==r?/**
 ...
 * slashes.
 *
 */function(e,t){let r=function(e,t){let r=t.split("/").filter(e=>e.length>0).join("/");return 0===e.length?r:e+"/"+r}(e._location.path,t),n=new c3(e._location.bucket,r);return new hf(e.storage,n)}(t,r):t)}(e,t));if(e instanceof hg)return new hf(e,t);throw c4("To use ref(service, url), the first argument must be a Storage instance.")}(e=ei(e),t)}eM(new es(hw,/**
 * Cloud Storage for Firebase
 *
 * @packageDocumentation
 */function(e,{instanceIdentifier:t}){let r=e.getProvider("app").getImmediate(),n=e.getProvider("auth-internal"),i=e.getProvider("app-check-internal");return new hg(r,n,i,t,eV)},"PUBLIC"/* ComponentType.PUBLIC */).setMultipleInstances(!0)),eq(hy,hv,""),eq(hy,hv,"esm2017");const hE="undefined"!=typeof crypto&&crypto.randomUUID&&crypto.randomUUID.bind(crypto);var hC={randomUUID:hE};const hb=new Uint8Array(16),hT=[];for(let e=0;e<256;++e)hT.push((e+256).toString(16).slice(1));var hI=function(e,t,n){if(hC.randomUUID&&!t&&!e)return hC.randomUUID();e=e||{};let i=e.random||(e.rng||function(){// lazy load so that environments that need to polyfill have a chance to do so
if(!r&&!// getRandomValues needs to be invoked in a context where "this" is a Crypto implementation.
(r="undefined"!=typeof crypto&&crypto.getRandomValues&&crypto.getRandomValues.bind(crypto)))throw Error("crypto.getRandomValues() not supported. See https://github.com/uuidjs/uuid#getrandomvalues-not-supported");return r(hb)})();// Per 4.4, set bits for version and `clock_seq_hi_and_reserved`
 ...
 * Security Rules.
 */function(e,t,r,n={}){var i;let s=(e=cu(e,cp))._getSettings(),a=`${t}:${r}`;if("firestore.googleapis.com"!==s.host&&s.host!==a&&iP("Host has been set in both settings() and connectFirestoreEmulator(), emulator host will be used."),e._setSettings(Object.assign(Object.assign({},s),{host:a,ssl:!1})),n.mockUserToken){let t,r;if("string"==typeof n.mockUserToken)t=n.mockUserToken,r=iA.MOCK_USER;else{// Let createMockUserToken validate first (catches common mistakes like
// invalid field "uid" and missing field "sub" / "user_id".)
t=J(n.mockUserToken,null===(i=e._app)||void 0===i?void 0:i.options.projectId);let s=n.mockUserToken.sub||n.mockUserToken.user_id;if(!s)throw new iF(iM.INVALID_ARGUMENT,"mockUserToken must contain 'sub' or 'user_id' field!");r=new iA(s)}e._authCredentials=new i$(new iB(t,r))}}(n,...e)}return n}(hS),hD=/**
 ...
 */function(e=ej(),t){e=ei(e);let r=eF(e,hw),n=r.getImmediate({identifier:void 0}),i=W("storage");return i&&/**
 ...
 * @public
 */function(e,t,r,n={}){!function(e,t,r,n={}){e.host=`${t}:${r}`,e._protocol="http";let{mockUserToken:i}=n;i&&(e._overrideAuthToken="string"==typeof i?i:J(i,e.app.options.projectId))}(e,t,r,n)}(n,...i),n}(hS);h_(hD),h_(hD,"images");class hk{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelFirebase is connected!")};get=async e=>{let t=await /**
 ...
 */function(e){e=cu(e,cf);let t=cu(e.firestore,cw),r=c_(t),n=new cY(t);return(/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 */function(e){if("L"/* LimitType.Last */===e.limitType&&0===e.explicitOrderBy.length)throw new iF(iM.UNIMPLEMENTED,"limitToLast() queries require specifying at least one orderBy() clause")}(e._query),(function(e,t,r={}){let n=new iU;return e.asyncQueue.enqueueAndForget(async()=>(function(e,t,r,n,i){let s=new l7({next:r=>{// Remove query first before passing event to user to avoid
    ...
    t.enqueueAndForget(()=>lL(e,a)),r.fromCache&&"server"===n.source?i.reject(new iF(iM.UNAVAILABLE,'Failed to get documents from server. (However, these documents may exist in the local cache. Run again without setting source to "server" to retrieve the cached documents.)')):i.resolve(r)},error:e=>i.reject(e)}),a=new lx(r,s,{includeMetadataChanges:!0,Z_:!0});return lN(e,a)})(await ci(e),e.asyncQueue,t,r,n)),n.promise})(r,e._query).then(r=>new cX(t,n,e,r)))}(function(e,t,...r){if(e=ei(e),co("collection","path",t),e instanceof cp){let n=iZ.fromString(t,...r);return cc(n),new cg(e,/* converter= */null,n)}{if(!(e instanceof cm||e instanceof cg))throw new iF(iM.INVALID_ARGUMENT,"Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");let n=e._path.child(iZ.fromString(t,...r));return cc(n),new cg(e.firestore,/* converter= */null,n)}}(hA,e)),r=[];// console.log(newCollection);
return t.forEach(e=>{// console.log(`docId: "${doc.id}"`);
let t={};t.docId=e.id;let n=e.data();for(let e in n)t[e]=n[e];r.push(t)}),"productItems"===e&&r.sort((e,t)=>e.itemName.localeCompare(t.itemName)),r};addDocToFirestore=async(e,t)=>{try{let r=cy(hA,e,t.docId);await function(e,t,r){var n;e=cu(e,cm);let i=cu(e.firestore,cw),s=(n=e.converter)?r&&(r.merge||r.mergeFields)?n.toFirestore(t,r):n.toFirestore(t):t;return cJ(i,[/** Parse document data from a set() call. */(function(e,t,r,n,i,s={}){let a,o;let l=e.wu(s.merge||s.mergeFields?2/* UserDataSource.MergeSet */:0/* UserDataSource.Set */,t,r,i);cF("Data must be an object, but it was:",l,n);let c=cx(n,l);if(s.merge)a=new sl(l.fieldMask),o=l.fieldTransforms;else if(s.mergeFields){let e=[];for(let n of s.mergeFields){let i=cU(t,n,r);if(!l.contains(i))throw new iF(iM.INVALID_ARGUMENT,`Field '${i}' is specified in your field mask but missing from your input data.`);cj(e,i)||e.push(i)}a=new sl(e),o=l.fieldTransforms.filter(e=>a.covers(e.field))}else a=null,o=l.fieldTransforms;return new cA(new sO(c),a,o)})(cR(i),"setDoc",e._key,s,null!==e.converter,r).toMutation(e._key,aA.none())])}(r,t),console.log("Item/document succesfully added with docId: ",t.docId)}catch(e){console.error("Error adding document: ",e)}};updateDocInFirestore=async(e,t)=>{try{let r=cy(hA,e,t.docId);await function(e,t,r,...n){e=cu(e,cm);let i=cu(e.firestore,cw),s=cR(i);return cJ(i,[("string"==typeof// performing validation.
(t=ei(t))||t instanceof cb?/** Parse update data from a list of field/value arguments. */function(e,t,r,n,i,s){let a=e.wu(1/* UserDataSource.Update */,t,r),o=[cU(t,n,r)],l=[i];if(s.length%2!=0)throw new iF(iM.INVALID_ARGUMENT,`Function ${t}() needs to be called with an even number of arguments that alternate between field names and values.`);for(let e=0;e<s.length;e+=2)o.push(cU(t,s[e])),l.push(s[e+1]);let c=[],h=sO.empty();// We iterate in reverse order to pick the last value for a field if the
// user specified the field multiple times.
for(let e=o.length-1;e>=0;--e)if(!cj(c,o[e])){let t=o[e],r=l[e];// For Compat types, we have to "extract" the underlying types before
// performing validation.
r=ei(r);let n=a.mu(t);if(r instanceof cP)c.push(t);else{let e=cO(r,n);null!=e&&(c.push(t),h.set(t,e))}}let u=new sl(c);return new cD(h,u,a.fieldTransforms)}(s,"updateDoc",e._key,t,void 0,n):/** Parse update data from an update() call. */function(e,t,r,n){let i=e.wu(1/* UserDataSource.Update */,t,r);cF("Data must be an object, but it was:",i,n);let s=[],a=sO.empty();st(n,(e,n)=>{let o=cV(t,e,r);// For Compat types, we have to "extract" the underlying types before
// performing validation.
n=ei(n);let l=i.mu(o);if(n instanceof cP)s.push(o);else{let e=cO(n,l);null!=e&&(s.push(o),a.set(o,e))}});let o=new sl(s);return new cD(a,o,i.fieldTransforms)}(s,"updateDoc",e._key,t)).toMutation(e._key,aA.exists(!0))])}(r,t),console.log("Item/document succesfully updated in Firestore, docId: ",t.docId)}catch(e){console.error("Error updating document in Firestore: ",e)}};deleteDocFromFirestore=async(e,t)=>{try{var r;await (r=cy(hA,e,t),cJ(cu(r.firestore,cw),[new aU(r._key,aA.none())]))}catch(e){console.error("Error deleting document: ",e)}};// Support Functions
// Generate unique ID
generateUniqueId=()=>hI();// Generate a unique alphanumeric Order Number array
generateOrderNumber=e=>{let t="ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789",r="";for(let n=0;n<e;n++){let e=Math.floor(Math.random()*t.length);r+=t[e]}return r};getLinkToImage=async e=>new Promise((t,r)=>{let n=h_(hD,`images/${e}`);/**
 * Returns the download URL for the given Reference.
 * @public
 * @returns A `Promise` that resolves with the download
 *     URL for this object.
 */(function(e){e._throwIfRoot("getDownloadURL");let t=function(e,t,r){let n=t.fullServerUrl(),i=ht(n,e.host,e._protocol),s=e.maxOperationRetryTime,a=new hh(i,"GET",function(t,n){let i=function(e,t,r){let n=hs(t);return null===n?null:function(e,t,r){let n={};n.type="file";let i=r.length;for(let e=0;e<i;e++){let i=r[e];n[i.local]=i.xform(n,t[i.server])}return Object.defineProperty(n,"ref",{get:function(){let t=n.bucket,r=n.fullPath,i=new c3(t,r);return e._makeStorageReference(i)}}),n}(e,n,r)}(e,n,r);return(/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
await this.getUpdateLocalDataBase(),hB.productItems)){let t=await this.modelFirebase.getLinkToImage(e.itemImg).then(e=>e).catch(e=>{console.log("Error getting download URL: ",e)});// FOR TEST AND TBS - REMOVE IN PROD:
...
this.renderCartAndOrdersSummary(hH,hq),this.attachEventListenrs()};// TO-DO: check where it is used?
...
getUpdateLocalDataBase=async()=>{for(let e of(console.log("Get or Update data from Firebase and keep in local dataBase{}."),hO))hB[e]=await this.modelFirebase.get(e),console.log(`[*** dataBase.${e} ***] :`,hB[e])};deleteObjectFromArrayByDocId=(e,t)=>{let r=e.filter(e=>e.docId===t);return -1!==r?(e.splice(r,1),console.log("Deleted element/document with docId = ",t)):console.log("Not found element/document with docId = ",t),e};// Assign to sessionIdCustomer an object/document associated with sesstionIdUser (by userId)
...
let t=hB.productItems.filter(t=>t.itemId===e);return 0===t.length?null:t[0]};getDocIdFromArrayByItemId=(e,t)=>{let r=e.filter(e=>e.itemId===t);return(// TEST-TBS - REMOVE FOR PROD
...
e.appendChild(this.viewOrders.createPaymentMethodContentBlock(hK,!0,hx));let t=this.viewOrders.createCheckOutCartWrapper(),r=document.createElement("div");for(let e of(r.classList="checkout-cart-items",hH)){let t=this.getProductObjectById(e.itemId);// Third argument ("checkout-cart-item") is a classList to pass to 'div' element
...
let t=this.getProdIdFromElementId(e.id),r=this.getDocIdFromArrayByItemId(hH,t);r?this.handleQtyChangeBtn(e,1):this.addProduct(t,hV),this.renderCartAndOrdersSummary(hH,hq)};// "Go to Cart" button at right panel or checkout panel
...
let t=JSON.parse(JSON.stringify(hH)),r=JSON.parse(JSON.stringify(e.orderId));for(let e in // TEST-TBS REMOVE IN PROD
...
console.log("Function: 'handleQtyChangeBtn()': update QTY of prod ID: ",e.id),console.log("'sessionIdCartItems': ",hH),console.log("itemId: ",r);// product/object to update:
let n=JSON.parse(JSON.stringify(hH.find(e=>e.itemId===r)));if(!n){console.log("Product/object not found");return}// remove product from array:
let i=hH.filter(e=>e.itemId!==r);n.qty+t>0?(this.updateProduct(i,n,t),this.updateCartDOM(r,n)):(this.deleteProduct(i,n),this.handleGotoCartBtn())};// Update payment method if new one is selected by User
updatePaymentMethodInSession=()=>{let e=this.handleRadioButtonSelection();null!==e?hK.paymentMethod!==e?(// Compare with current value
hK.paymentMethod=e,console.log(`Updated paymentMethod to ${e}`)):console.log("paymentMethod is the same. No update needed."):console.log("No radio button selected. Cannot update paymentMethod.")};addProduct=(e,t)=>{let r={docId:this.handleNewIdGeneration(),itemId:e,qty:1,userId:t};console.log("MODEL: try to add to database.cartsData a [*** newProductToCart *** ]: ",r),this.handleAddDocToFirestore("cartsData",r),hH.push(r),console.log("Updated [*** sessionIdCartItems ***]: ",hH)};updateProduct=(e,t,r)=>{// Modify product/object
