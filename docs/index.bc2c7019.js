...
return(""+e[2]).replace(/^((?:https?|file|ftp|(chrome|moz|safari-web)-extension):\/\/.+)\/[^/]+$/,"$1")+"/"}return"/"}(),c[e]=t),t}}),i("4G3qk").register(JSON.parse('{"ceDhw":"index.bc2c7019.js","4M8Gf":"icon-add-order.d5dcde0a.png","kSZwU":"icon-cart.dd455062.png","8HtV0":"icon-order.5ea58f4f.png","baUMc":"admin.7eece7fc.js"}'));var o={};o=i("260Jh").getBundleURL("ceDhw")+i("4G3qk").resolve("4M8Gf");var a={};a=i("260Jh").getBundleURL("ceDhw")+i("4G3qk").resolve("kSZwU");class d{constructor(e){this.controller=e}// FOR TEST AND TBS - REMOVE FOR PROD
...
i.appendChild(a),l.appendChild(s),l.appendChild(m),l.appendChild(u),l.appendChild(p),l.appendChild(h),l.appendChild(C),l.appendChild(g),l.appendChild(L),l.appendChild(_),l.appendChild(E),l.appendChild(I),f.appendChild(y),f.appendChild(w),d.appendChild(l),d.appendChild(f),c.appendChild(i),c.appendChild(d),k.appendChild(v),k.appendChild(b),r.appendChild(n),r.appendChild(c),r.appendChild(k),r}}// import iconAddOrder from "../../other_assets/icon-add-order.png";
...
console.log(`ACTION NEEDED: DELETE PRODUCT WITH ID ${n.itemId} FROM CART.`)):(n.qty+=r,this.controller.handleViewOfPartQty(t,n.qty)));return e}}class L{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelOrders is connected!")}}class _{constructor(){}checkModuleLinkage=()=>{console.log("HELLO, ModelUser is connected!")};getSessionIdCustomerByCustomerId=(e,t)=>{let r=t.filter(t=>t.custId===e);return r[0]}}var E=i("ljOiA");// Init database instance (dataBase):
let I={},f="",y={},w={},k={},v={},b={items:8e5,handling:8e3,total:808e3},P=document.createElement("div");P.innerHTML="";let B=document.createElement("div");B.innerHTML="";let T=document.createElement("div");T.innerHTML="";// Init cross-ref Map: image - url
...
k=this.getOrdersByUserID(f),console.log(`Orders History [*** sessionIdOrders ***] for User ID ${f}: `,k),// Right Container - Render Cart Summary
this.renderCartAndOrdersSummary(v,k),this.attachEventListenrs()};// Get sessionIdCustomer data by customer's ID
getSessionIdCustomerData=()=>{w=this.modelUser.getSessionIdCustomerByCustomerId(y.custId,I.customersData)};// Clear container
...
handleGotoProductsBtn=()=>{this.renderProductItemsList(I.productItems),this.renderCartAndOrdersSummary(v,k)};// "Add to Cart" button at detailed product card (page)
...
this.modelCart.updateCartPriceVariable(v,b,I.productItems,w.custHandlingFee),this.renderCartSummary(v),this.renderCartSummaryWithPrice(v,b)};// "Proceed to checkout" button at right panel
...
console.log("Received command to display PRODUCT CARD with ID: ",e.id),console.log("Clicked inside product item");let t=this.getProdIdFromElementId(e.id),r=this.getProductObjectById(t);this.renderProductCard(r),this.renderCartAndOrdersSummary(v,k)};handleDisplayHistoricalOrder=e=>{console.log("Received command to display ORDER with ID: ",e.id)};handleQtyDeductBtn=(e,t)=>{console.log("Received command to deduct qty of product with ID: ",e.id);let r=this.getProdIdFromElementId(e.id);console.log("itemId: ",r),console.log("sessionIdCard: ",this.modelCart.changeQtyInCart(v,r,-1)),this.handleGotoCartBtn()};handleQtyAddBtn=(e,t)=>{console.log("Received command to add qty of product with ID: ",e.id);let r=this.getProdIdFromElementId(e.id);console.log("itemId: ",r),console.log("sessionIdCard: ",this.modelCart.changeQtyInCart(v,r,1)),this.handleGotoCartBtn()};handleButtonsClickLeft=(e,t)=>{let r=e.target;// "< Back to Products" button
